...
- **Setup and distribution** — `setup-symlinks.sh` and its helpers, which wire the repo into `~/.claude-agent/` and `~/.cursor/`, merge the policy settings, and install the reminder and git hooks; `doctor.sh` confirms the runtime is ready.
