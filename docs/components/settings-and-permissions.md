...
**Grant channel.** A code-executing or write grant scoped to ONE plan stage's ONE spawned child — neither fleet-wide (so not `settings/base.json`) nor durable across stages (so not a per-kind baseline) — is declared in that stage's `[stage.grants]` table or derived automatically from its own `verify_command`/output artifacts, validated by `scripts/agentctl/grants.py`, and materialized only into that child's own `--settings`/`--add-dir`. See `scripts/agentctl/README.md` § `[stage.grants]` for the declaration shape and derivation rules, and [[settings-permission-tiers]] § Grant channel for the never-grantable forms, approval binding, and named residuals.
