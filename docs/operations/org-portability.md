...
| `retry_detector_allowlist` | unset (org-neutral commands only) | your VCS's read verbs | **Comma**-separated (not whitespace — a command prefix contains spaces) command prefixes `hook-retry-detector.py` treats as cheap-to-repeat, on top of Core's own list (`ls`, `pwd`, `git status`/`log`/`diff`, `grep`, `rg`, …). Entries are matched literally, not as regexes. Set e.g. `hg status,hg log`. |
| `commit_command_verbs` | unset (`git` only) | your VCS's front-end | Comma/space-separated tool names whose `<tool> commit` the `hook-readme-currency-reminder.py` advisory watches, on top of `git`. Pairs with `changeset-vcs.local` below: the key decides *when* the hook fires, the program supplies *what changed*. Set e.g. `hg`. |
...
Four seams are machine-local **files** rather than `agent-identity.local` keys, because their payload is a list or a program rather than a scalar:

- `<config root>/canon-roots.local` — extra canon-read-only path roots, one per line. See [cross-session-scope-isolation.md](cross-session-scope-isolation.md).
- `<config root>/skill-first-classes.local` (`$CLAUDE_SKILL_FIRST_CLASSES_FILE`) — a JSON array of `{"name", "pattern", "skill"}` objects extending `hook-skill-first.py` beyond its single Core builtin (`tracker`). Which command verbs count as a hand-rolled domain operation is an org fact, so Core holds the matcher and the deployment holds the verbs. Patterns match case-insensitively and each entry fails open on its own — a bad regex costs its nudge, never the Bash call.
- `<config root>/session-digest-vcs.local` (`$CLAUDE_SESSION_DIGEST_VCS`) — an executable emitting the VCS section of `session-start-digest.sh`, run with the project root as `$1`. git is the only VCS Core knows; when this program exits 0 its output replaces Core's git section, and on any non-zero exit the digest falls back to git.
- `<config root>/changeset-vcs.local` (`$CLAUDE_CHANGESET_VCS`) — an executable printing a repo root then one changed path per line, run in the directory the commit targets. `hook-readme-currency-reminder.py` asks git first and falls back to this; a non-zero exit means "not my VCS". Pairs with `commit_command_verbs` above.
