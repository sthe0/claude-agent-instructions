...
Two seams are machine-local **files** rather than `agent-identity.local` keys, because their payload is a list rather than a scalar: `<config root>/canon-roots.local` (extra canon-read-only path roots, one per line — see [cross-session-scope-isolation.md](cross-session-scope-isolation.md)) and `<config root>/skill-first-classes.local`. The latter is a JSON array of `{"name", "pattern", "skill"}` objects extending `hook-skill-first.py` beyond its single Core builtin (`tracker`): which command verbs count as a hand-rolled domain operation is an org fact, so Core holds the matcher and the deployment holds the verbs. Patterns are matched case-insensitively and each entry fails open on its own — a bad regex costs its nudge, never the Bash call.
