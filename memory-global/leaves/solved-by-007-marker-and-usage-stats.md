...
**The marker.** `scripts/agentctl/solved_marker.py` defines `SOLVED_MARKER = "solved_by_007"` (the literal codename never appears — see CLAUDE.md's org-neutral convention) and a dispatcher `stamp(tracker_key)` that classifies the key via `looks_like_key`: a Startrek-shaped key (`^[A-Z][A-Z0-9]+-\d+$`, identical to `agentctl.classify.TRACKER_KEY_RE`) routes to `difficulty_channel.adapters.startrek.add_tag` (`PATCH /v2/issues/<key>` with the **add** operator `{"tags": {"add": [tag]}}`, never a full-array replace, so co-existing tags survive); a fully-qualified GitHub ref (`owner/repo#N`, `owner/repo/issues/N`, or a full URL) routes to `adapters.github.add_label` (`POST /repos/<repo>/issues/<n>/labels` with `[label]`). Both verbs reuse the adapters' existing auth/HTTP helpers rather than a new client. `stamp` never raises: no key, no token, an HTTP error, an unclassifiable key, or a **bare** GitHub number (rejected rather than silently defaulted to some repo) all resolve to `{"stamped": False, "skipped_reason": ...}`.
...
- [[tracker-write-token]] — the Startrek write-auth mechanics (`~/.tracker-token`) `add_tag` reuses.
