...
description: "The 3-tier instruction-development tracking model (Core / Org / Project): each tier has a proactive internal backlog + a reactive report inbox, with a collapse rule when filers==editors and a stream-separation axis (separate queue vs separate label); Core venue is PUBLIC — split mixed items by tier at filing time, pre-publish check via check-org-neutral.py."
type: reference
schema: leaf/v1
created: 2026-06-30
...
| **Org** (org-wide) | **ORG-BACKLOG** | **ORG-REPORT** | no — two queues |
| **Project** (a single deployed project) | **PROJECT-QUEUE** | PROJECT-QUEUE | collapsed — one queue |
...
- **Org** — org-specific-but-cross-project refinements. Split into two queues because the populations differ: internal org *users* file **ORG-REPORT** reports (they hit difficulties but don't own org instructions), org *authors* plan in **ORG-BACKLOG**. An internal-tracker difficulty-channel adapter targets the report queue.
- **Project** — project-specific refinements (e.g. a single deployed project). A project participant **designates one queue** via a machine-readable field (`agent-project.json` → `{"instruction_queue":"PROJECT-QUEUE"}`, discoverable from project cwd); because participants have edit rights, backlog and reports collapse onto it. The queue is structural config, not prose — `file-difficulty.py` resolves it from the field; classifying *which tier a target belongs to* is the model's cognition, the queue *lookup* is the structure.
...
- **Two trackers** (Org) — the **queues themselves** separate the flows: ORG-BACKLOG = backlog, ORG-REPORT = reports. Nothing more needed.
...
- **Collapsed** (Project) — one queue, one population; sub-distinguish by label/component within PROJECT-QUEUE only if a project later wants the split.
