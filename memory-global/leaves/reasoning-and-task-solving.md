...
- **On a fresh session opened directly on a ticket/task** (no prior dialogue to lean on — e.g. entered via a `--new`/task-entry launcher) drive this step yourself, proactively: restate the goal, surface every ambiguity as an explicit question, and keep asking until your understanding is clearly correct **and the user has confirmed it** — do not wait to be redirected mid-flight. A **bare-title ticket whose description field is still empty** is a direct, checkable signal that this step has not happened yet, not permission to start decomposing. Once understanding is captured, write it into the ticket's **description** field (not only into comments) — that field is the checkable artifact that the step actually ran. (User correction, ticket-driven task, 2026-08-27: skipped this at a fresh `--new` session start; the ticket's empty description was the tell.)
