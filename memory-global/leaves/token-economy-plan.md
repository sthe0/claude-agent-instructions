...
| 7 | Default **Sonnet** for `developer`/`thinker`/`tech-writer`/`yandex-cloud-expert` spawns (planner inherits parent) | community + [Anthropic costs](https://code.claude.com/docs/en/costs) | `scripts/spawn-specialist.py` `MODEL_BY_KIND` + `--model` override | **done 2026-06-04** |
...
| 12 | Default window `opus` (200k), not `opus[1m]`; reserve 1M for tasks that need it | Update 2026-06-04 (≈4× `cache_read` on 1m long sessions) | `~/.claude/settings.json` `model` (machine-local) | **done 2026-06-04** (this machine; set on others via `/config`) |
...
| 18 | Trim idle MCP servers per mount; prefer CLI over MCP | [Anthropic costs — MCP overhead](https://code.claude.com/docs/en/costs) | `/mcp` + per-mount config | **planned** — 5 servers (yandex/tracker/wiki/intrasearch/logos) load globally every session |
