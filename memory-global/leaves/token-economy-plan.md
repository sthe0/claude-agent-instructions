...
Drafted 2026-05-27 after analysing four recent project sessions (three-digit ticket IDs, one repeated) and cross-referencing with public best-practices on context engineering (Anthropic, LangChain, arXiv SkillReducer / Tokenomics, community Claude Code guides). Two-layer derivation: **local measurement** + **web research**; sources cited inline. **Revised 2026-06-04** after a full sweep of all mount transcripts — see § Update 2026-06-04, which reframes the dominant driver.
...
|---|---|---|---|---|---|---|
| Session A | 646k | **185M** | 2.17M | 245 | 42 | `workflow.py` ×10 |
| Session B | 624k | **177M** | 2.81M | 224 | 38 | `test_quality.py` ×10 |
| Session C | 160k | 33M | 1.53M | 51 | 15 | a CLI entry-point module ×2 |
...
| 2 | Volatile sections at bottom of MEMORY.md | strict-prefix caching ([Anthropic API docs](https://platform.claude.com/docs/en/build-with-claude/prompt-caching)) | Restructure of project MEMORY.md files | done for one project — extend per project when adopted |
...
| 7 | **Complexity-driven** sub-agent model: `--complexity low/medium/high` → haiku/sonnet/opus, set by the manager per spawn from the assigned task's difficulty. `--model` and `--complexity` are a required, mutually exclusive pair — no per-kind default, no inherit-parent fallback (`MODEL_BY_KIND` removed 2026-08-26); an optional `--max-complexity` caps the classified tier, default unset (no cap). | community + [Anthropic costs](https://code.claude.com/docs/en/costs) | `scripts/spawn-specialist.py` `COMPLEXITY_MODEL`/`resolve_model`; rubric in `--help` | **done 2026-06-04, revised 2026-08-26** |
...
| 12 | **Arbitrary context-size cap.** Effective cap = `CLAUDE_AUTOCOMPACT_PCT_OVERRIDE`% × active window, where `CLAUDE_CODE_DISABLE_1M_CONTEXT` selects the window (200k vs 1M). The override only *lowers* the trigger below the ~83% default (Math.min clamp), so the enforceable max is ~830k. `scripts/set-context-cap.sh <tokens>` computes both knobs into `settings/base.json` (e.g. 150k→200k window/75%; 300k→1M/30%). The `model: opus`=200k alias is version/plan-dependent and proved wrong here (2026-06-09 `/context` showed `167k/1m`, session reached 214k) — not a reliable lever. Compaction is a trigger (soft cap, minor overshoot), env applies on new sessions only. | Update 2026-06-04 (≈4× `cache_read` on 1m long sessions) + 2026-06-09 user requests (cap; then arbitrary cap) | `scripts/set-context-cap.sh` → `CLAUDE_CODE_DISABLE_1M_CONTEXT` + `CLAUDE_AUTOCOMPACT_PCT_OVERRIDE` in `settings/base.json` (all machines via `apply-settings.sh`) | **done 2026-06-09** — verify next session `/context` shows the chosen window/cap |
...
| 18 | Trim idle MCP servers per mount; prefer skills over MCP | [Anthropic costs — MCP overhead](https://code.claude.com/docs/en/costs) | global generic skill>MCP rule (`skill-first-dispatch.md` + CLAUDE.md); env-specific mapping in project memory (community skills > MCP for tracker/orchestration-platform); **one MCP server moved to project-scope** in `~/.claude.json` (loads only under that project's cwd) | **partial 2026-06-04** — that one server done; further per-mount trimming of remaining org-specific MCP servers planned |
| 19 | An internal engineering-practices skill's conventions: selective memory recall (manifest → ≤5 facts), task context to disk (pass by path, not inline), time-boxed investigation, strict 10-line log cap | an internal skill bundle providing shared engineering practices | log cap already in `CLAUDE.md`; recall + context-to-disk | **partial** — log cap done; recall/context-to-disk planned |
| 20 | **API-level (the underlying product's own quota, not our CC quota):** server-side compaction `compact_20260112`, context-editing `clear_tool_uses_20250919` (+`clear_at_least`), Batch API −50% (stacks with caching) | [context editing](https://platform.claude.com/docs/en/build-with-claude/context-editing), [compaction](https://platform.claude.com/docs/en/build-with-claude/compaction), [batch](https://platform.claude.com/docs/en/build-with-claude/batch-processing) | that product's own code (separate contour) | **noted** — flag to product; out of our-quota scope |
...
| 22 | **Policy effectiveness/efficiency tracker** — operationalizes this leaf's feedback loop (was just `/usage` + `cost-report.py`, hand-run): a per-session ledger + `policy-scorecard.py` measuring tokens/$/cache-share/$-per-session *and* policy compliance (spawn model mix, inherit→opus rate, missed-delegation clusters) with trend vs the previous window and a Flags section. A throttled weekly SessionStart nudge prompts the review; a Flags-fire → `self-improvement` → record-metric-movement procedure closes the loop empirically. Makes every token-economy rule here measurable instead of exhortation-only. | this leaf + [[delegatable-work-patterns]] 2026-06-17 audit (policy unmeasured) | `scripts/policy-scorecard.py` + `scripts/hook-policy-scorecard-due.py` + `~/.local/log/claude-policy-ledger.jsonl` + [[policy-effectiveness-tracking]] | **done 2026-06-17** |
