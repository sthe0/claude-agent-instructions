...
**A local filesystem path is not a reference for an external reader** (a ticket comment, PR description, Slack message, an internal wiki article) — the reader's client cannot open a path that only exists on the agent's own machine. When an artifact needs to be visible there, attach/upload the actual file, or link a URL the reader's own account can reach — never paste the local path into the comment text as if it were a link. A bare local path is fine only in an internal report to the user in the same session, where the filesystem is actually shared;
