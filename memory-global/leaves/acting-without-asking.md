...
**Never the agent's own permission surface — no plan can declare it.** To achieve *a permission gate that still means something after a plan is approved*, treat a permission-layer denial as a **stop-and-ask** and never as an obstacle to route around: do not edit `~/.claude-agent/settings*.json`, a project `.claude/settings*.json`, or any `permissions.allow` list in order to make the denied action succeed. This holds even when the denied action is itself squarely inside the approved plan — approval sanctions *the action*, never *a widening of the rules that judge the action*. A mechanized gate for this rule (a three-conjunct PreToolUse hook: widens a permission surface, an arming denial already stands in-session, and the widening would have covered that denial) was built and landed 2026-09-16 (commit `b1d5802`), then reverted the next day, ~22 hours later (2026-09-17, commit `7584e4d`) after it denied ordinary Bash calls (`git`, `python3`) across parallel sessions on unvalidated false-positive risk — its false-positive rate against real, representative session traffic was never measured before landing. For now this rule is enforced by judgement only.
...
Mechanizable half: "an `Edit`/`Write` to a `settings*.json` that adds entries to `permissions.allow`" is decidable from observable inputs, so it belongs in a `hook-guard-*` PreToolUse gate rather than in prose alone (template: `scripts/hook-guard-canon-readonly.py`). Such a gate (hook-guard-permission-self-grant.py, without a `scripts/` prefix or backticks — see the paragraph above) was attempted and abandoned; until a replacement lands, this paragraph's rule is enforced by judgement only.
