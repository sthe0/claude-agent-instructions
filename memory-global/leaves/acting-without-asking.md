...
**Never the agent's own permission surface — no plan can declare it.** To achieve *a permission gate that still means something after a plan is approved*, treat a permission-layer denial as a **stop-and-ask** and never as an obstacle to route around: do not edit `~/.claude-agent/settings*.json`, a project `.claude/settings*.json`, or any `permissions.allow` list in order to make the denied action succeed. This holds even when the denied action is itself squarely inside the approved plan — approval sanctions *the action*, never *a widening of the rules that judge the action*. `scripts/hook-guard-permission-self-grant.py` is the mechanized half of this rule. It is **fail-closed**, not simply narrower: a conjunct it cannot **evaluate** never earns an allow. What keeps it from refusing every ordinary permissions edit is **relevance**, its third conjunct: the gate fires only when an entry being added would have *covered* a denial already standing in this session, not merely because a denial and a widening both occurred somewhere in a long session. Which call shapes land on which side of that is the module's own docstring and test file, not this leaf — see `scripts/hook-guard-permission-self-grant.py`.
...
Mechanizable half: "an `Edit`/`Write` to a `settings*.json` that adds entries to `permissions.allow`" is decidable from observable inputs, so it belongs in a `hook-guard-*` PreToolUse gate rather than in prose alone (template: `scripts/hook-guard-canon-readonly.py`). That hook now exists — `scripts/hook-guard-permission-self-grant.py` — and mechanizes this paragraph's rule only in part; where it does not reach, the prose rule still applies unmechanized. Within its reach it errs fail-closed, refusing what it cannot establish to be safe.
...
- **Treating a "don't touch `<path>`" constraint as a write-only exclusion.** § 1 pre-authorizes `Read`/`Grep` in general, but when a plan, checkpoint, or the user names a path as off-limits ("не трогать X", "do not touch X") without further qualification, default to reading that as full scope exclusion — no `Read`/`Grep`/diff-comparison either, and no delegating that reading to a subagent for reference or comparison. Only narrow it to a write-lock when the constraint itself says so ("don't edit", "read-only is fine"). (User correction 2026-08-25: a fork agent was told to `Read` a file under a standing off-limits path for comparison against a permitted sibling path — user: "вообще не надо туда лезть!")
