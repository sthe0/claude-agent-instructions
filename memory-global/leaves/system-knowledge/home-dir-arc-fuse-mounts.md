...
A machine-local guard enforces this: `scripts/hook-arc-mount-search-guard.py` (PreToolUse `Bash|Grep|Glob`) reads the live mount table and **denies** a recursive search whose resolved root spans ≥2 `fuse.arc` mounts, with a message to re-scope. See also [[delegatable-work-patterns]] (pin the search root when delegating Pattern-B exploration).
