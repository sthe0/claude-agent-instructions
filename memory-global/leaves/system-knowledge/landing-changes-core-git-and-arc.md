...
- Invocation points mechanized in [PR 14261837](https://a.yandex-team.ru/review/14261837): `hook-resolution-land-arc.py` gains an fs-only residue scan (no arc shell-outs; `ARC_LAND_MOUNT_POINTS_FILE`/`ARC_LAND_STORES_DIR` test seams) that nudges GC at the resolution gate, and `backends/arc.sh` gains the missing `backend_teardown_workspace()` verb (anchor/root + dirty-store guards, `CLAUDE_DRY_RUN=1` preview) with the thin CLI `teardown-arc-mount.sh <name>` — one-command teardown of a finished task mount.
