...
- **R4 fix (2026-09-25, `convergence-levers-r1-r2-r4` plan, stage 2): the judge now runs strictly
  ...
  green result against a content identity git itself computes: a disposable copy of the venue's
  real index (never the real one), `git add -A` + `git write-tree` against that copy via a
  `GIT_INDEX_FILE=<tmp>` env prefix, digested together with HEAD sha; fails safe (`None`, so the
  check always re-runs) on any non-zero/empty git call or on a gitlink (mode 160000) in the temp
  index. A repeated `record-result` on an unchanged tree skips re-running the command but still
  re-queries the judge every time (the judge is never cached, only the mechanical check is). Two
  documented limits: the cache assumes the check reads only the venue tree — a check that also
  reads files outside it (the plan TOML, `proposals.json`, ...) can be served stale even though its
  own venue is unchanged; and a change confined to a `.gitignore`d file is invisible to `git add -A`
  exactly as it would be to a real commit, so it never invalidates the cache. Second, and more directly
