...
### 13. A criterion's file-path literals are never re-diffed against the live repo tree — `stat` them, not just the script around them

Norm 7 dry-runs a `verify_command`'s **script mechanics** (syntax, a called function's real signature). This is the same authoring gap applied to the **path literals** the script's mechanics operate on — a distinct failure class, because a grep/path check can be syntactically perfect and still name a target that never existed in the shape the plan assumes.

A stage's `verify_command`, `output_artifacts`, and `material_refs` routinely freeze a file path at authoring time from a mental model of the repo layout (a flat module, a single file) — a model that review checks for internal *consistency* (does the plan's own story hang together) but never diffs against the live tree, because nothing in `submit_plan`/`approve`/a thinker review actually runs `stat`/`ls` on the literal. A layout the plan never matched at all (the file was already a package, not a flat module, at the moment the plan was authored) survives identically to a layout that drifts mid-execution (norm 6's class) — both are invisible to every review round for the same reason: a quoted path reads as a fact, not as a claim requiring its own check.
