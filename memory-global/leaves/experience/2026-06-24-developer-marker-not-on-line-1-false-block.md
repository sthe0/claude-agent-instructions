...
**Common:** Spawn returns a terminal line the wrapper does not accept as a marker (MALFORMED) and agentctl would route to BLOCKED, but the underlying work fully or partially succeeded; manager must independently re-verify the artifact (arc status/log/diff/pr, re-run tests) rather than trust the marker, then finish only the residual steps.

**Variations:** Cause of the rejected marker differs: 2026-06-24 = prose preamble before the marker (formatting); 2026-06-25 = budget exhaustion / mid-flight error so no marker emitted at all; 2026-07-11 = marker correctly formatted and well-placed but its *token* absent from the known `RETURN_MARKERS` set (compounded by the set being duplicated across 4 copies with a drift-guard covering only 2). Recovery for the first two is identical (verify artifact → finish residual in-thread → record-result); the budget case adds a calibration fix (raise small-spawn budget to medium), and the set-gap case is fixed at the source (add the token to every copy + widen the drift-guard) rather than per-occurrence.
