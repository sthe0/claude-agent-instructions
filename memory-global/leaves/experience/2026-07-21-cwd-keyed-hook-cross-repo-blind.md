...
- Where it arose: Core instructions repo (~/claude-agent-instructions), scripts/hook-resolution-reminder.py, coordinated from a non-Core project session.
...
- Non-obvious finding worth the leaf: hook-readme-currency-reminder's COMMIT_RE matches only a CONTIGUOUS `git commit` or the internal VCS CLI's `commit` invocation, so it never fires on `git -C <dir> commit` — the `-C` branch of effective_git_cwd is UNREACHABLE through this hook (it is reachable through hook-guard-canon-readonly, whose _is_git_commit tokenizes). So the hook's cross-repo cases are cd+git / cd+the internal VCS CLI / no-redirect fallback; the `-C` branch is covered at the lib level (test_git_cwd.py). A planned `-C` hook test was swapped for this reason (plan refinement, applied in-thread).
