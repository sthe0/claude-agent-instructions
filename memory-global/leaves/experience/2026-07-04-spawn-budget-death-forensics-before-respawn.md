...
## Difficulty
Desired: a budget-dead spawn's completed/partial work is recognized and reused. Actual: MALFORMED/no-marker + BLOCKED node reads as 'stage failed', inviting a from-scratch respawn that redoes done work and doubles cost — or worse, a 'fix' of code that was already correct. The same marker-less return is also produced by a judge-call **abort** ('Connection closed mid-response'), which raises the sharper question of whether the judge **ledger** can hold a false verdict for a call that never completed — worked as its own context below (2026-08-10).
...
### 2026-08-10 — JUDGE-CALL ABORT, verdict V4, and the remainder the ledger cannot yet position
- Where it arose: task `judge-import-blindness-and-norm-debt`, Stage 3's investigation into whether an aborted judge call could leave a false verdict in `judge_ledger`. Full finding: [/home/the0/.local/state/claude-judge-budget/connection-closed-finding.md](/home/the0/.local/state/claude-judge-budget/connection-closed-finding.md) — **machine-local and outside any repository** (this host only, hand-authored in Russian, so committing it would need a language exception); the load-bearing numbers are carried in this block so the record stands without it. The norm debt closed here was opened by [[2026-08-05-deferring-ask-gate-and-the-binding-timeout-layer]], whose own largest finding — a fail-open judge has no way to report its own non-execution — is the ledger-side twin of the verdict below. It lands **here** rather than there because the manifestation is a marker-less child return, which is this leaf's subject; a reader arriving from the judge-timeout side should follow the pointer that leaf now carries.
- **Stage 3's verdict, carried verbatim: V4 — "the observation object lies outside this ledger."** The judge ledger records what `_classify` (declaration in `scripts/agentctl/advisor.py`, around line 368) decided from a completed `runner(...)` result; a connection abort that never produces a result for `_classify` to classify has, by construction, nothing to record — the ledger's absence of a `malformed` row is not evidence the abort didn't happen, it is evidence the abort happened somewhere the ledger was never wired to see.
- **The remainder — the enumeration of connection-abort manifestation paths grew from four to six during Stage 3's review, and two of the six are NOT resolved by this verdict.** The first four are covered by V4 as stated. The two new paths are explicitly named here as **NOT excluded but UNPOSITIONABLE** — each is a genuine abort-adjacent defect this task surfaced, but neither has a home in this ledger's own model of what it observes, and each awaits a separate future difficulty rather than being closed here:
  1. **The return-marker extractor in `scripts/lib/marker_extract.py`** (around line 216) launches `claude -p` for return-marker extraction as a bare `run([...])` call with no `judge_ledger` wrapping at all — an abort on THIS call leaves no trace in any ledger, not even the fail-open kind `_classify` produces, because nothing here calls `_classify`.
  2. **An abort arriving AFTER the first response line is indistinguishable from an honest verdict**, because `_classify` (advisor.py:397-401) parses only `lines[0]` of the collected stdout — a connection that stays open long enough to emit one well-formed `YES`/`NO` line and then drops produces the exact same `(verdict, reason, malformed=False, timed_out)` tuple as a judge that ran to completion and meant it. **The load-bearing premise, which must travel with the claim:** `_classify` returns early on `if result.returncode != 0` (advisor.py:392-393), *before* the `lines[0]` parse, so path 2 exists only because the aborted client exits **rc=0** with truncated stdout. That premise was established by reading the client bundle's stream-abort path — on an abort with output already emitted it keeps the emitted content blocks, appends a synthesized error text after them, and exits through the normal generator path — not by staging a live abort; it is a code-read claim, and it is load-bearing. A reader who checks `_classify` alone will otherwise conclude the non-zero branch already closes this path.
- **The claim this block must not reproduce.** Stage 3's investigation initially framed "zero `malformed` rows in the ledger" as evidence "no aborts occurred inside the judge" — that claim is refuted and must not recur: zero `malformed` only negates aborts that happen BEFORE the first meaningful token reaches `_classify`'s parser (the `not lines` branch, advisor.py:395-396); it says nothing about an abort after that point (path 2 above) and nothing about a call `_classify` never sees at all (path 1 above).
