...
- Where it arose: the shared `~/arcadia_claude_local` vcs mount (project `.claude/` storage), checked out to a live parallel session's feature branch `users/the0/claude-agents` (their open PR #14175489, commit a564e1ab). I committed my project SI fix **on that branch**, so `a vcs pr create` did not open a new PR — it **stacked my commit onto their open PR**, polluting it (and their next merge attempt went `merge_failed`).
- Prevention (the rule, restated for arc): in a shared arc tree, derive your branch off **trunk** (`a vcs checkout -b users/<login>/<slug> trunk`) BEFORE committing — never commit on the parallel session's feature branch. `a vcs push -u <branch>` to create a same-named user ref (a branch created off trunk tracks trunk, so a bare vcs push aims at trunk and is `Forbidden`).
- Recovery (already entangled): (1) `a vcs checkout -b <fresh> trunk` + `arc cherry-pick <my-sha>` → isolate; (2) `a vcs push -u <fresh>` → own PR via `a vcs pr create --publish` → owner fast-path `a vcs pr merge --now --force <id>` (merged as r20156524); (3) **re-verify the shared branch server head is still your stacked SHA immediately before** force-restoring (`the vcs log <branch>`), then `a vcs checkout <shared-branch>` + `arc reset --hard <parallel-SHA>` + `a vcs push --force` → un-pollutes their PR (drops only your commit; their commit preserved → PR returns to `open`).
- Consequence to handle: `arc reset --hard <parallel-SHA>` reverts your now-landed-on-trunk files **in the working tree**, so the composed `.claude/` live hooks go stale until the mount next reaches a trunk-inclusive state. Verify the plan's final_checks against **trunk content** (`a vcs show trunk:<path>`), not the reverted working tree, then restore the tree to the parallel session's clean SHA. The delivered fix is live in this specific mount only once the parallel branch merges/rebases onto trunk — surface that as a coordination note, it is not a task failure.
