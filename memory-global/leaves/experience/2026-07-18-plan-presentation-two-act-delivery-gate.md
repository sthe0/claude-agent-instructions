---
name: 2026-07-18-plan-presentation-two-act-delivery-gate
description: Approval could fire without the user ever seeing the plan because 'show the plan' was agent perception, not a verifiable mechanism. Fix: split approval into two machine-enforced acts — (1) present-plan --kind essence registers a receipt and hook-plan-delivery-gate.py stamps it ONLY when the registered rendering actually landed as a completed turn's FINAL assistant text (pre-tool-call text never renders, so the final message is the only observable), (2) the approval AskUserQuestion must carry a [show-full-plan] option; plan_presentation_blockers denies cmd_approve until the stamp exists. Two non-obvious gotchas cost real debug time: the harness STRIPS the trailing newline from a delivered text message, so the registered rendering file must be rstrip('\n')ed or the delivered-vs-registered sha mismatches and the gate denies; and the ask must open in a CLEAN turn (no preceding tool call) or the turn-split gate drops it. The gate proved itself at runtime by gating this very session's own approval.
type: reference
schema: difficulty/v1
generality: 0
resolution_confirmed_by_user: "fedor.solovyev"
refs: [2026-06-29-agentctl-verify-venue-worktree-needs-substantive-replan, 2026-07-03-engine-replan-artifact-discipline, 2026-07-09-gate-must-execute-what-it-attests, 2026-07-04-topological-gate-when-signal-unobservable]
created: 2026-07-18
last_verified: 2026-09-13
---

# Plan presentation as a machine-verified two-act delivery gate (not agent perception)

## Difficulty
A user could click 'approve' on a plan whose essence they never saw: 'presenting the plan' was left to agent judgement (perception), with no mechanism forcing the essence to be shown before the confirmation ask, and no separation between presenting and confirming.

## Order & criterion
classify -> submit-plan -> present-plan --kind essence (emit the registered rendering as the turn's FINAL message) -> [next turn] approval AskUserQuestion carrying [show-full-plan] -> hook stamps delivery -> approve (blocked until stamped). Presentation and confirmation are two SEPARATE acts by construction.

**Acceptance check:** measurable: 52 delivery-gate tests + 5 negative end-state checks green (same-turn ask denied; never-delivered ask denied+unstamped; delivered ask allowed+stamped; degraded-missing-transcript fail-open unstamped); AND the mechanism gated this session's own approval (runtime dogfood).

## Contexts

### 2026-07-18 — initial
- Where it arose: agentctl coordination engine (present-plan/plan_presentation_blockers in cli.py+gates.py, delivery.py sidecar, hook-plan-delivery-gate.py); CLAUDE.md § Approved plan; tech-writer charter widened Russian-only -> dialogue language.
- Working plan: 5 stages: (1) buy CLAUDE.md byte headroom by extracting elaboration to a leaf; (2) PlanPresentation receipt + delivery sidecar + present-plan cmd + plan_presentation_blockers wired into cmd_approve; (3) hook-plan-delivery-gate.py denies the approval ask until the registered essence landed as a completed turn's FINAL message and stamps at that moment; (4) widen tech-writer charter to dialogue language; (5) re-norm prose (Approved-plan definition, ask-user-question-split-turn overclaim, planner/coordinator procedure).

### 2026-07-20 — follow-up: byte-exact match hardened to two-tier normalized
- Difficulty (this leaf's own trailing-newline gotcha, promoted from operator-discipline to mechanism): the delivered-vs-registered match at hook-plan-delivery-gate.py was byte-exact (`receipt.rendering_text in text`), so ANY incidental reformat between the registered essence and the delivered final message — a stripped trailing `\n`, collapsed whitespace, a casefold difference — tripped `_NOT_DELIVERED_REASON`, forcing an extra re-emit + verification cycle between the plan and the confirmation question (the user-reported "several bash commands, some with errors" before the ask). The bug bit THIS follow-up's own approval ask (trailing-newline sha mismatch denied it — live-reproduced the exact defect being fixed).
- Fix: two-tier match — exact substring first (unchanged strong fast path), then a normalized substring via `text_shape.normalize_string` (casefold + collapse whitespace), the SAME helper already used for the invariant substring check in gates.py:497. Genuinely missing CONTENT (a dropped word/line) still fails both tiers and still DENIES — normalization tolerates reformatting, not omission, so the "essence actually landed" guarantee is intact. The sleep-2 turn-split timer and ask-split hook are untouched (load-bearing, work around real client text-loss).
- 2-stage substantive plan; S1 spawn:developer (two-tier predicate + docstring/comment refresh + EXTENDED existing tests, no new file), S2 measurable verify. Diff vs merge-base = 4 files (+83/-3): hook + 2 test files + 1 config-root-refs allowlist line (drift left by the 13th-instance memory commit 10dffe0). pytest gate suites 39/39 + turn-end 68/68 green; verify-agentctl/readme OK. Lesson reinforced: a gotcha named in an experience leaf ("rstrip the rendering file or the sha mismatches") is a standing signal to MECHANIZE the tolerance, not to keep relying on operator discipline — the next operator who forgets the rstrip hits the same re-emit cycle.


### 2026-09-13 — byte-exact copy + isMeta autonomous-loop-tick exclusion
- Where it arose: session 7dd14604-9e20-4e2f-9e8d-4020748ea221 (heredoc-body-spans stage-5 re-approval, GitHub claude-agent-instructions#108)
- Working plan: essence-delivery text must be copied byte-exact from the registered receipt.rendering_text (whitespace/casefold-normalized match is the only tolerance) — a paraphrase reconstructed from memory/summary fails both match tiers in _delivery_observed. Separately, confirmed by reading lib/transcript_turns.py's _is_real_user_prompt directly (not by hypothesis): any transcript entry carrying isMeta truthy is excluded from counting as a real user-prompt boundary ('if entry.get("isMeta"): return False'), so an 'Autonomous loop check'/'Autonomous loop tick' synthetic prompt can NEVER close a delivery segment, by design — only a genuine background-task queued_command-shaped notification closes one. This corrects an earlier wrong hypothesis (that _content_items fails to wrap bare-string message.content) which was disproven by reading the actual source; _content_items wraps a bare string correctly. Verified live: emitting the byte-exact essence text as the sole content of a turn (zero tool calls), followed by a genuine background-timer completion, produced FOUND VALID DELIVERY: True via delivered_final_texts().

## Common core & variations
**Common:** presenting a plan's essence is not agent perception but a mechanically-checked fact: the exact registered `receipt.rendering_text` bytes (tolerant only of whitespace/casefold reformatting) must appear as a completed turn's FINAL assistant message with no following tool call, in an entry the transcript scanner counts as a real turn boundary. Every context so far is one more way that chain silently breaks: a stripped trailing newline (07-20), a paraphrase from memory instead of a verbatim copy, or a synthetic `isMeta` prompt (an autonomous-loop tick) that can never close a segment by design (09-13).

**Variations:** the 07-18/07-20 contexts are mechanism bugs (fixed in code — two-tier normalized match). The 09-13 context is an **operator-discipline** finding, not a code defect: `_is_real_user_prompt`'s `isMeta` exclusion is correct, deliberate design, so the fix there is behavioral (copy the receipt text verbatim; never treat an autonomous-loop tick as a delivery-closing turn), not a patch to the gate.

## Cost
Engine-attributed spawn cost ~$21 list-price (3 spawns; flat-Max telemetry, not real money — see [[flat-max-billing-cost-framing]]); ~69 min wall-clock; 5 stages, 2 failed stage-results, 3 replans, 3 difficulty records. The rework mass sat almost entirely in the verify-final false-fail cycle (Stage-1 execution-time-target trap + whole-repo final_check red on out-of-deliverable issues) and the delivery-gate self-debug (trailing-newline sha mismatch, turn-split denial), not in the mechanism design.

## Self-critique of the agent system
The verify-final false-fails I hit (Stage-1 verify_command re-running an execution-time byte-headroom sub-target that a LATER stage was DESIGNED to consume; whole-repo final_check red on 2 out-of-deliverable issues) are a 9th context of the already-richly-recorded verify-scoping family [[2026-06-29-agentctl-verify-venue-worktree-needs-substantive-replan]] / [[2026-07-03-engine-replan-artifact-discipline]] ('verify_commands must check DURABLE properties, not time-bound tree states'). Root planner defect: verify_commands/final_checks were authored from memory without dry-running against the post-all-stages baseline. The novel, code-invisible bits worth reading are the trailing-newline delivery-match gotcha and the two-act design rationale.
