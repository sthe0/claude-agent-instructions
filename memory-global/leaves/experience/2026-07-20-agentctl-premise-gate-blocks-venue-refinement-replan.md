...
### 2026-07-23 — spawn-cwd/pytest/quality self-improvement, substantive replan to a NEW plan file (v1->v2->v3)
...
**Variations:** Nuances across occurrences. (1) COVERAGE gate has a SECOND clause: when critique.differences_to_remove is non-empty, replan_coverage_blockers ALSO requires the (means,method) multiset across stages to DIFFER from the approved snapshot — folding invariants into the invariants field is not enough; add a 'verification-means refinement' sentence to a stage method naming the change. Recipe step (3) should read: fold preserved invariants into conditions/invariants AND record the changed means in a stage means/method. (2) The mandatory plan-review SPAWN could not Read the plan at ~/.claude-agent/plans/<slug>.toml (PERMISSION-REQUEST) because spawn-specialist.py anchors the child's filesystem isolation on the LAUNCH cwd; remedy = launch spawn-specialist.py from a PARENT cwd that contains BOTH ~/.claude-agent/plans/ AND the repo — a simpler alternative to the venue leaf's --add-dir ~/.claude-agent/plans fix. (3) No state.json surgery needed when enumerate raises 0 candidates — the whole cycle goes through the normal CLI verbs (confirming the e987fe3 fix + clean-refinement path); delivered 6 canon writers stamping via edit_ledger.stamp, landed 127a26c. (4) Bootstrap wrinkle (spawn-cwd/pytest occurrence): a self-modifying fix granting a spawned developer a Bash allow (pytest) cannot have its own spawn:developer stages self-verify by running that command until the fix is live — resolved by the coordinator running the measurable verify itself (it holds the rights) while the FINAL stage proves runtime behaviour via a fresh live spawn launched through the now-fixed spawn-specialist.py, with a second pre-existing-allow probe (python3 -m json.tool) discriminating --settings MERGE (both run) from REPLACE (control blocks).
