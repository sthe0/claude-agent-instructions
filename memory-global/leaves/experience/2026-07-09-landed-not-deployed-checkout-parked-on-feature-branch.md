...
Feature/bench work had been done *in the primary checkout itself* rather than in a throwaway linked worktree, leaving the primary on that branch after the session ended. Preventive rule (candidate for instructions, not yet filed): do instruction-repo feature/bench work in a **linked worktree**, keep the primary checkout tracking main, so the live hooks never drift. When multiple checkouts exist, their hook code must be homogeneous — verify with a grep of `settings.json` for stray non-primary paths.
