...
- Where it arose: ~/claude-agent-instructions; 8 audited defects: proc_tree.py, sync-instructions-repo.sh, doctor.sh, verify-instructions-sync.sh, cursor/scripts/{migrate-cursor-namespace,install-cursor-links}.sh, install-sync-systemd-timer.sh, setup-ccgram.sh, hook-multi-mount-search-guard.py
