...
- **`apply-settings.sh` is additive — dropping a key from base does NOT remove it from live.** `env = base.env + live.env` with live winning on conflict. So (a) a stale live value silently shadows base (the drift that caused the overshoot), and (b) when you *remove* a key from `base.json` (as `fc7c5ce` removed `pct`/`DISABLE_1M`) you must also delete it from `~/.claude-agent/settings.json` by hand or it persists. Always verify the live `env` after applying. For **hook** registrations specifically (not `env` keys), `install-reminder-hooks.sh` now covers the removal direction itself — it prunes an owned-and-missing hook entry from both `$CLAUDE_AGENT_HOME/settings.json` and (prune-only) `~/.claude/settings.json` — so a deleted hook script's registration does not persist the way a dropped `env` key does; see [docs/operations/guards.md](../../docs/operations/guards.md) § Live machine state.
