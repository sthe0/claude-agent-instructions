...
- **Author, on driving the review to mergeable.** Opening the review is the *start* of the author's ownership, not the end. Until the change is **mergeable** — CI green, every reviewer comment answered, required approval obtained — the author drives it there **unprompted**: watch the review's checks, new comments, and approval; fix what CI or reviewers flag; re-check. Do not wait for the user to say *"the tests failed in your review"*. Reuse the zero-token mechanism in [[long-job-monitoring]] § Generalization — a detached poller emitting `CHECK_FAILED` / `NEW_COMMENTS` / `MERGED`, closure gated on the merged (or mergeable-and-approval-obtained) marker: *"submitted" ≠ "mergeable"*. Only reading a review comment and fixing the code stays in the model; the monitoring cadence and marker-emission are the poller's job, not the user's. A functional fix made after the owning stage already passed needs its own review pass, not just the original one — `gates.code_review_blockers` binds once, at the moment a `spawn:developer` stage is first recorded `passed`, and never re-fires on later edits to that stage's files. Addressing live PR-review comments routinely touches already-passed code after that binding, so a review-comment fix that changes actual logic/behavior (not a doc/typo/rename) needs a fresh `code-reviewer` pass — or, when a full pass is disproportionate to the fix's size, an explicit self-review note recorded in the PR reply — before it counts done. Precedent (2026-09-25, project memory): two functional fixes landed after their owning stage already passed, with no re-review at all; the original code-reviewer pass had even flagged one of the eventual human-review findings but scoped it out as "predates this diff" — a call that did not survive human review.
...
- [[determinize-required-specialist-dispatch]] — the mechanism (`review_dispatch` plugin) that pairs a `spawn:developer` stage's `dispatch` event with `gates.code_review_blockers`; its binding is one-shot at first-passage, which is exactly the gap the post-stage-fix sentence above names.
