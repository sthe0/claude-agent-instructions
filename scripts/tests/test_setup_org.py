...
def test_creates_identity_and_prints_checklist(tmp_path):
    ...
    idf = tmp_path / ".claude" / "agent-identity.local"
    assert idf.exists()
...
def test_idempotent_never_overwrites(tmp_path):
    ...
    content1 = (tmp_path / ".claude" / "agent-identity.local").read_text()
    r2 = _run(tmp_path, "--non-interactive")
    assert r2.returncode == 0, r2.stderr
    content2 = (tmp_path / ".claude" / "agent-identity.local").read_text()
