...
arming at approve, re-derivation on every replan branch, sub-plan custody at the CLI
layer, and the quality-ledger surface — the ACTING half effort.py's own module
docstring hands to cli.py. effort.py's pure estimate/actual/divergence layer (the
ranking fix, both re-arm belts, the CUSTODY sentinel) is pinned in test_effort.py;
this file drives the same mechanism through the real commands so a fire actually
routes the session into DIAGNOSING with no user question asked.
"""
...
def test_substantive_replan_rederives_the_estimate_over_the_new_stage_list(store, fixtures_dir):
    ...
    # (1 initial + 3 developers + 0 replans since baseline) at medium (3.00 each).
    assert state.effort_estimate[effort.SCALE_SPEND] == pytest.approx(3 * 3.00 + 4 * 3.00)
