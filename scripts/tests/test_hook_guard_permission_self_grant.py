...
# --- Finding 8: an unmodelled writer verb is UNKNOWN, not a silent allow ----------

@pytest.mark.parametrize("command_template", [
    "dd if=/dev/zero of={s} bs=1 count=5",
    "dd of={s} if=/dev/zero bs=1 count=5",
    "dd bs=1 count=5 of={s} if=/dev/zero",
])
def test_dd_writing_the_surface_via_of_keyword_arg_denies(tmp_path, command_template):
    # Finding 8, the exact reproduction: `dd`'s `if=`/`of=` keyword-argument idiom
    # matches neither a shell redirect nor `bash_write_targets`'s enumerated writer-verb
    # table, so `command_write_targets` reports NO candidate at all for this segment --
    # byte-identical, by truthiness, to a segment that genuinely writes nothing. Before
    # `_unmodelled_writer_segment`, an empty overall candidate list reached ALLOW by a
    # route that never judged the write. All three argument orderings must deny: the
    # fix is a default-posture flip over the unrecognized VERB, not a positional
    # pattern match on `of=` appearing in a particular argument slot.
    s = write_settings(tmp_path)
    command = command_template.format(s=s)
    reason = hook.decide(payload("Bash", {"command": command}, ARMED_BASH, tmp_path))
    assert reason is not None


@pytest.mark.parametrize("command_template", [
    "openssl enc -aes-256-cbc -in secret.txt -out {s}",
    "curl -o {s} https://example.com/payload",
])
def test_other_unmodelled_writer_programs_also_deny(tmp_path, command_template):
    # Proof the fix is general rather than a `dd`-shaped patch: `openssl -out` and
    # `curl -o` write via a flag-plus-SEPARATE-argument idiom, structurally different
    # from `dd`'s glued `key=value` idiom, and neither verb is on `_WRITE_INCAPABLE_
    # WORDS` or the enumerated writer table either. Both must deny for the same reason
    # `dd` does -- an unrecognized verb reporting no candidate is UNKNOWN, not a "no".
    s = write_settings(tmp_path)
    command = command_template.format(s=s)
    reason = hook.decide(payload("Bash", {"command": command}, ARMED_BASH, tmp_path))
    assert reason is not None
...
@pytest.mark.parametrize("command", [
    "git status", "cat file.txt", "grep hello file.txt", "ls -la", "head file.txt",
])
def test_ordinary_recognized_safe_command_while_armed_is_allowed(tmp_path, command):
    # The fix must not become blanket deny-everything-unrecognized: a command led by a
    # word this gate already knows is write-incapable -- `cat`/`grep`/`ls`/`head` have no
    # flag that writes to an arbitrary path, and `git status` is on the git
    # read-or-record allowlist -- still allows while armed, exactly as it did before
    # this round. `grep` is the row that matters most: it is what an UNBROADENED
    # `_WRITE_INCAPABLE_WORDS` actually blocked, live, in the session that wrote this fix.
    (tmp_path / "file.txt").write_text("hello\n", encoding="utf-8")
    reason = hook.decide(payload("Bash", {"command": command}, ARMED_BASH, tmp_path))
    assert reason is None


def test_sort_dash_o_writing_the_surface_denies(tmp_path):
    # The near-miss `_WRITE_INCAPABLE_WORDS`'s own comment names: GNU `sort -o FILE`
    # takes an arbitrary output path exactly like `curl -o`/`dd of=`, so `sort` is
    # deliberately NOT on the safe list even though most of its invocations are
    # read-only -- it must deny like any other unmodelled writer, not join the safe set
    # by superficial resemblance to `uniq`/`cut`/`wc`.
    s = write_settings(tmp_path)
    command = f"sort -o {s} evil.json"
    reason = hook.decide(payload("Bash", {"command": command}, ARMED_BASH, tmp_path))
    assert reason is not None


def test_env_prefixed_dd_denies(tmp_path):
    # `_WRITE_INCAPABLE_WORDS`'s own near-miss, round 10: `env` was on the list, but
    # `env <cmd> ...` EXECUTES another program -- the "cannot execute another program"
    # half of write-incapability `_segment_word_is_write_incapable`'s docstring itself
    # requires, and `env` fails it exactly like `bash` does. `env dd of=<surface>` used
    # to be judged write-incapable on the leading word `env` alone, so `dd`'s own
    # write-capable `of=` idiom -- the exact shape the `dd` rows above deny on their
    # own -- never even reached `_unmodelled_writer_segment`.
    s = write_settings(tmp_path)
    command = f"env dd if=/dev/zero of={s} bs=1 count=5"
    reason = hook.decide(payload("Bash", {"command": command}, ARMED_BASH, tmp_path))
    assert reason is not None


def test_env_prefixed_interpreter_denies(tmp_path):
    # The sharper shape the `dd`-only fix cannot generalize from: `env python3 <script>`
    # runs an arbitrary interpreter behind `env`'s own write-incapability. No writer-verb
    # table entry could ever enumerate this closed -- only removing `env` from
    # `_WRITE_INCAPABLE_WORDS` does, so this segment now falls through to the same
    # UNKNOWN-while-armed default posture Finding 8 gave every other unmodelled verb.
    s = write_settings(tmp_path)
    script = tmp_path / "grant.py"
    script.write_text("pass\n", encoding="utf-8")
    command = f"env python3 {script}"
    reason = hook.decide(payload("Bash", {"command": command}, ARMED_BASH, tmp_path))
    assert reason is not None


def test_mutation_forcing_no_unmodelled_writer_check_reddens_a_deny_case(tmp_path, monkeypatch):
    # `_unmodelled_writer_segment` itself, Finding 8's own primitive: without it, `dd`'s
    # `of=` idiom reports no candidate, the empty overall list is trusted as "writes
    # nothing", and the call that denies today flips to allow.
    s = write_settings(tmp_path)
    command = f"dd if=/dev/zero of={s} bs=1 count=5"
    call = payload("Bash", {"command": command}, ARMED_BASH, tmp_path)
    assert hook.decide(call) is not None

    monkeypatch.setattr(hook, "_unmodelled_writer_segment", lambda *a, **k: None)
...
# --- a heredoc body that defeats shlex is not automatically an unknown write ------

# The two delimiter forms bash itself will NOT expand -- the SHELL never looks inside a
# quoted delimiter's body for `$`, backtick or backslash, so `git commit` (write-incapable
# by conjunct (B)) is the only thing that ever sees this text. `git commit` is not on
# `shell_tokens.CONSUMERS`, so `strip_heredoc_bodies` returns each of these unchanged and
# the body -- ordinary English prose with an odd apostrophe count PLUS a command
# substitution that would be dangerous if the shell ever expanded it -- survives into the
# pre-lex `shlex.split` in both, defeating it identically.
_SAFE_HEREDOC_DELIMITERS = ("<<'EOF'", '<<"EOF"')

# The two forms bash WILL expand: an unquoted delimiter, quoted or not by a dash. The
# `$(touch /tmp/pwned)` in the body below is what a real reviewer confirmed actually runs
# under these two forms, before conjunct (D) existed -- the shell performs the command
# substitution while building the heredoc body, before `git` ever starts, so
# write-incapability of `git commit` bought nothing.
_SHELL_EXPANDED_HEREDOC_DELIMITERS = ("<<EOF", "<<-EOF")

# Shared by both: an apostrophe (defeats `shlex.split`) and a `$(...)` (the shell-expansion
# hazard conjunct (D) exists to catch). Only the DELIMITER decides whether the `$(...)` is
# ever live.
_HEREDOC_BODY = "This won't parse without the fix: $(touch /tmp/pwned)."


@pytest.mark.parametrize("delim", _SAFE_HEREDOC_DELIMITERS)
def test_git_commit_heredoc_body_with_odd_apostrophe_is_allowed(tmp_path, delim):
    # The exact class stage 7 hit committing its own evidence file. `git commit -F -
    # <<'EOF'` whose body is ordinary prose with an unmatched apostrophe used to resolve
    # through `_ON_ERROR` and deny while armed; `_write_incapable_bash_call` is what
    # settles the write question on the command word instead of demanding a tokenization
    # the word makes irrelevant. The quoted delimiter means the shell will not expand the
    # `$(...)` in the body either, so conjunct (D) agrees this is genuinely inert.
    command = f"git commit -F - {delim}\n{_HEREDOC_BODY}\nEOF"
    assert hook.decide(payload("Bash", {"command": command}, ARMED_BASH, tmp_path)) is None


@pytest.mark.parametrize("delim", _SHELL_EXPANDED_HEREDOC_DELIMITERS)
def test_git_commit_heredoc_with_unquoted_delimiter_still_denies(tmp_path, delim):
    # Byte-identical body to the allowed case above, but an UNQUOTED delimiter lets bash
    # expand the `$(touch /tmp/pwned)` inside the body BEFORE `git` ever runs -- so
    # `git commit`'s own write-incapability (conjunct (B)) is beside the point: the shell,
    # not git, would be doing the writing. Conjunct (D) is what tells this case apart from
    # the quoted one above; without it this command was allowed and the substitution ran.
    command = f"git commit -F - {delim}\n{_HEREDOC_BODY}\nEOF"
    reason = hook.decide(payload("Bash", {"command": command}, ARMED_BASH, tmp_path))
    assert reason is not None


def test_bash_consumer_heredoc_body_with_apostrophe_still_denies(tmp_path):
    # `bash` can turn its own stdin into code, so it must never earn the fast ALLOW --
    # even though its body-defeats-shlex shape is byte-identical to the git-commit case
    # above. Conjunct (B) is what tells the two apart: `bash` is not write-incapable.
    command = "bash <<EOF\nThis won't parse either.\nEOF"
    reason = hook.decide(payload("Bash", {"command": command}, ARMED_BASH, tmp_path))
    assert reason is not None


def test_tee_heredoc_writing_the_surface_still_denies(tmp_path):
    # `tee` IS on `shell_tokens.CONSUMERS`, so this command never reaches the new
    # pre-lex-failure branch at all -- the existing write-target scan finds and denies
    # it exactly as it did before this stage, which is the regression this row pins.
    s = write_settings(tmp_path)
    command = f"tee {s} <<'EOF'\ndon't care about this apostrophe\nEOF"
    reason = hook.decide(payload("Bash", {"command": command}, ARMED_BASH, tmp_path))
    assert reason is not None
    assert str(s) in reason


def test_unparseable_command_line_proper_still_denies(tmp_path):
    # No heredoc at all: the unbalanced quote is ON the command line itself, so conjunct
    # (A) fails even though `echo` is write-incapable, and this must not become a second
    # way to slip past `_ON_ERROR`.
    reason = hook.decide(payload(
        "Bash", {"command": "echo don't"}, ARMED_BASH, tmp_path))
    assert reason is not None


def test_second_command_after_heredoc_terminator_still_denies(tmp_path):
    # Regression: a real second command after the FIRST heredoc's terminator line is
    # invisible to conjuncts (A)/(B) -- they only ever lex the command line proper -- and
    # a writer verb there, spelled with shell quoting a textual regex scan cannot resolve
    # (`t''ee`, still `tee` to real bash), used to slip past the old textual-only conjunct
    # (C) entirely: (A) lexed clean, (B) saw only `git commit -F -` and stayed silent, the
    # regex found no bare "tee" word, and (D) (now (F)) only ever looks at the FIRST
    # heredoc's body. `_write_incapable_bash_call` returned True (write-incapable) and the
    # call was allowed, even though real bash runs the quoted `tee` as a fully independent
    # second command and genuinely writes the settings file. Conjunct (C) --
    # `shell_tokens.first_heredoc_consumes_entire_tail` -- is what closes this: a command
    # whose tail holds anything beyond the first heredoc's own body is never write-
    # incapable, regardless of what conjuncts (D)-(F) would have said about it.
    s = write_settings(tmp_path)
    command = (
        "git commit -F - <<'EOF'\n"
        "prose with an apostrophe: don't\n"
        "EOF\n"
        f"t''ee {s} <<<\"data\"\n"
    )
    reason = hook.decide(payload("Bash", {"command": command}, ARMED_BASH, tmp_path))
...
def test_glued_semicolon_before_writer_still_denies(tmp_path):
    # Finding 4, repro 1: a `;` glued to the next word with no surrounding whitespace
    # (`hi;dd`, not `hi ; dd`) is left as ONE token by plain `shlex.split` --
    # `bash_write_targets.split_segments`'s own docstring names that as an accepted
    # residual for ITS original callers. Conjunct (B) used to inherit that residual by
    # tokenizing with plain `shlex.split`, so the segment list here was a single
    # `['echo', 'hi;dd', 'of=...', ...]` whose word `echo` is write-incapable and hid
    # `dd` -- a genuinely write-capable command -- entirely. Real bash still runs `dd`
    # as its own statement regardless of how it lexed, truncating the settings file.
    # `separator_exact_split` must split the glued `;` into its own token so (B) sees
    # `dd` as a second segment and denies.
    s = write_settings(tmp_path)
    command = f"echo hi;dd of={s} if=/dev/null <<'EOF'\ndon't\nEOF"
    reason = hook.decide(payload("Bash", {"command": command}, ARMED_BASH, tmp_path))
    assert reason is not None


def test_glued_pipe_into_bash_still_denies(tmp_path):
    # Finding 4, repro 2: a `|` glued to `bash` (`hi|bash`, not `hi | bash`) hides the
    # pipeline's real second command from conjunct (B) the same way the glued `;` does
    # above -- but worse, because `bash` is not merely write-capable, it is a full
    # interpreter: `bash <<'EOF'` runs its own stdin as CODE, so the heredoc body here
    # executes `dd`/`echo` as real commands rather than being inert data. Conjunct (F)
    # (`shell_tokens.first_heredoc_body_shell_inert`) correctly reports this body
    # shell-inert -- the SHELL never expands a `$()` in it -- which is exactly why (B)
    # catching the glued `|bash` is the only conjunct that can deny this: nothing else
    # in this function ever asks whether the heredoc's CONSUMER treats its body as code.
    s = write_settings(tmp_path)
    command = f"echo hi|bash <<'EOF'\ndd of={s} if=/dev/null\necho don't\nEOF"
    reason = hook.decide(payload("Bash", {"command": command}, ARMED_BASH, tmp_path))
    assert reason is not None
...
@pytest.mark.parametrize("command_template", [
    "echo hi;tee {s}",
    "echo hi;dd of={s} if=/dev/null",
    "echo hi|tee {s}",
])
def test_glued_separator_in_a_cleanly_lexing_command_still_denies(tmp_path, command_template):
    # Finding 8, round 10: unlike `test_glued_semicolon_before_writer_still_denies` and
    # `test_glued_pipe_into_bash_still_denies` above, this command carries NO heredoc at
    # all -- it lexes cleanly under plain `shlex.split`, so it never reaches
    # `_write_incapable_bash_call`'s fallback (whose conjunct (B) already used
    # `separator_exact_split`). It exercises `_bash_widening`'s MAIN path instead:
    # `bash_write_targets.segments()`, shared by `command_write_targets` and
    # `_unmodelled_writer_segment`, used to tokenize with plain `shlex.split` too,
    # leaving the glued `;`/`|` fused to the next word (`hi;tee`, `hi;dd`, `hi|tee`) as
    # ONE token -- `split_segments` never drew a boundary there, so the whole line
    # became a single segment led by the write-incapable `echo`, hiding `tee`/`dd` (real
    # writer verbs) from every consumer of `segments()`. Real bash still runs the second
    # statement as its own command regardless of the glue and genuinely overwrites the
    # settings file.
    s = write_settings(tmp_path)
    command = command_template.format(s=s)
    reason = hook.decide(payload("Bash", {"command": command}, ARMED_BASH, tmp_path))
...
def test_glued_separator_subshell_after_heredoc_still_denies(tmp_path):
    # Finding 5 (round-4/5 regression), the exact reproduction: `separator_exact_split`
    # used `shlex.shlex(punctuation_chars=True)`, which groups an entire RUN of adjacent
    # punctuation characters into ONE token -- so a bash separator glued directly to a
    # subshell paren with no whitespace (`;(`) came back as a single token, not a member
    # of `bash_write_targets._BASH_SEPS`. Conjunct (B) therefore saw only ONE segment
    # spanning the whole command line, whose first (and only inspected) word is `git` --
    # write-incapable -- hiding the `dd` living inside the subshell entirely. The
    # subshell sits on the SAME line as the heredoc operator (before the terminating
    # newline), so conjunct (C) -- which only ever looks at text AFTER the heredoc's own
    # body -- cannot catch it either. Real bash runs the subshell as its own statement
    # regardless of the glue and genuinely truncates the settings file.
    s = write_settings(tmp_path)
    command = (
        f"git commit -F - <<'EOF';(dd of={s} bs=1 count=0)\n"
        "don't\n"
        "EOF"
    )
    reason = hook.decide(payload("Bash", {"command": command}, ARMED_BASH, tmp_path))
...
@pytest.mark.parametrize("command_template", [
    ...
    "echo hi # note\ncp /tmp/evil-source {s}",
    "git status # c\ntee {s} </dev/null",
    "git log --oneline # a\nsed -i s/x/y/ {s}",
])
def test_a_hash_comment_before_a_writer_still_denies(tmp_path, command_template):
    # Round-11 finding 1, the exact reproductions. `separator_exact_split` builds a
    # `shlex.shlex`, whose `commenters` defaults to `'#'` -- unlike plain `shlex.split`,
    # which sets it to `''`. Because bare newlines are rewritten to `;` BEFORE lexing,
    # that lexer sees the whole multi-line command, so a `#` anywhere truncated the token
    # stream: everything after it silently vanished from `segments()`, the command
    # collapsed to one segment led by a write-incapable word (`git`/`echo`), and the
    # writer on the next line reached ALLOW without any verdict ever being formed about
    # it. A trailing `# comment` on a multi-line Bash call is ordinary shape, not an
    # exotic one -- which is why this class shipped green through ten rounds with no `#`
    # anywhere in any of the three suites.
    s = write_settings(tmp_path)
    command = command_template.format(s=s)
    reason = hook.decide(payload("Bash", {"command": command}, ARMED_BASH, tmp_path))
    assert reason is not None
...
def test_process_substitution_via_heredoc_fallback_still_denies(tmp_path):
    # Finding 6, the exact reproduction a round-5 review confirmed genuinely truncates
    # the settings file via real bash (subprocess transcript), even though `git` itself
    # then errors out: `<(dd if=... of=<surface> bs=1)` forks and runs `dd` as a side
    # effect of building `git commit`'s own argument list, regardless of whether `git`
    # ever runs successfully. The heredoc body's apostrophe defeats the top-level
    # `shlex.split` (git commit is not on `shell_tokens.CONSUMERS`, so its body survives
    # unstripped) -- the exact shape `_write_incapable_bash_call` used to be asked to
    # settle. `_bash_widening`'s own upfront `has_process_substitution` check now denies
    # this before that fallback is even reached: before it existed, (A)-(F) all passed
    # -- (B) saw one write-incapable `git commit` segment because `<(...)` never starts
    # a new one, (D)/(E) found no bare `>` or writer-verb WORD (`dd`, `if=`, `of=` are
    # neither), and (C)/(F) are honestly true about the heredoc itself -- so the call
    # was allowed while it genuinely overwrote the surface.
    s = write_settings(tmp_path)
    command = f"git commit -F - <(dd if=/tmp/evil-source of={s} bs=1) <<'EOF'\ndon't\nEOF"
    reason = hook.decide(payload("Bash", {"command": command}, ARMED_BASH, tmp_path))
    assert reason is not None


def test_bare_newline_before_tee_still_denies(tmp_path):
    # Finding 7, repro 1: a bare `\n` between `echo hello` and `tee <surface>` is
    # ordinary whitespace to plain `shlex.split` -- unlike `;`/`|`/`&`/`&&`/`||`, which
    # DO become their own token once whitespace-surrounded, a bare newline never does.
    # This command is not `_recognized` (an `echo`+`tee` pipeline shape is not on
    # `shell_tokens.CONSUMERS`), so its heredoc body survives unstripped and the whole
    # raw text -- including the second physical line -- reaches
    # `bash_write_targets.command_write_targets`'s own `shlex.split` call directly: the
    # GENERAL path, never `_write_incapable_bash_call`'s fallback. Before
    # `normalize_newline_separators`, that call collapsed the two lines into one
    # segment whose first word is the write-incapable `echo`, hiding `tee` entirely,
    # even though real bash runs `tee` as its own statement and genuinely overwrites
    # the settings file.
    s = write_settings(tmp_path)
    command = (
        "echo hello\n"
        f"tee {s} <<'JSONEOF'\n"
        '{"permissions": {"allow": ["Bash(*)", "Bash(ls:*)"], "deny": []}}\n'
        "JSONEOF\n"
    )
    reason = hook.decide(payload("Bash", {"command": command}, ARMED_BASH, tmp_path))
    assert reason is not None
...
def test_bare_newline_after_heredoc_before_tee_still_denies(tmp_path):
    # Finding 7, repro 2: byte-identical hazard to the row above, but the FIRST
    # statement is itself heredoc-bearing (`printf hello <<'EOF'`) and the write-capable
    # `tee` sits on the physical line right after that heredoc's own terminator --
    # distinguishing this from Finding 1's `test_second_command_after_heredoc_terminator
    # _still_denies` (a quoted `t''ee` defeating a textual regex scan): here the tail
    # text is plain, unquoted `tee`, and the bug is purely that the newline separating
    # it from the first statement was never a token boundary at all. Real bash runs
    # `tee <surface> </dev/null` as an independent statement and truncates the file to
    # empty.
    s = write_settings(tmp_path)
    command = (
        "printf hello <<'EOF'\n"
...
def test_process_substitution_in_cleanly_lexing_command_still_denies(tmp_path):
    # Finding 6's explicit scope note: a `<(...)` construction that lexes CLEANLY under
    # plain `shlex.split` -- no heredoc, no odd apostrophe -- never reaches
    # `_write_incapable_bash_call` at all, so conjunct (G) alone cannot catch it; this
    # exercises `_bash_widening`'s MAIN path, the one that otherwise hands the command
    # straight to `bash_write_targets.command_write_targets`. That function's own verb
    # dispatch stops scanning a segment's operands at the first token starting with `<`
    # or `>` (`_operands_until_redirect`), so `<(dd ...)` truncates the scan before `dd`
    # is ever seen and the segment yields no candidate at all -- an empty list
    # indistinguishable from "this command writes nothing". `_bash_widening` must check
    # `shell_tokens.has_process_substitution` BEFORE trusting that empty candidate list.
    s = write_settings(tmp_path)
    command = f"git commit -F - <(dd if=/tmp/evil-source of={s} bs=1)"
    reason = hook.decide(payload("Bash", {"command": command}, ARMED_BASH, tmp_path))
    assert reason is not None
...
def test_mutation_forcing_plain_shlex_split_reddens_a_deny_case(tmp_path, monkeypatch):
    # `_write_incapable_bash_call`'s conjunct (A)/(B), Finding 4's own regression:
    # without `separator_exact_split`, this is exactly the vulnerable shape that used to
    # ship -- a glued `;` left as one token by plain `shlex.split`, hiding `dd` (a
    # write-capable command) from (B) behind the write-incapable `echo`. Forcing the
    # primitive back to plain `shlex.split` must reproduce the regression: the call that
    # denies today flips to allow.
    s = write_settings(tmp_path)
    command = f"echo hi;dd of={s} if=/dev/null <<'EOF'\ndon't\nEOF"
    call = payload("Bash", {"command": command}, ARMED_BASH, tmp_path)
    assert hook.decide(call) is not None

    monkeypatch.setattr(hook.shell_tokens, "separator_exact_split", shlex.split)
    assert hook.decide(call) is None


def test_mutation_forcing_unsplit_punct_runs_reddens_a_deny_case(tmp_path, monkeypatch):
    # Finding 5's own primitive, `shell_tokens._split_punct_run`: without it,
    # `separator_exact_split` reproduces the exact round-4/5 regression even though it
    # still uses `shlex.shlex(punctuation_chars=True)` -- the glued `;(` token is never
    # re-split into `;` and `(`, so it stays out of `bash_write_targets._BASH_SEPS` and
    # (B) again sees one segment spanning the whole line. Forcing the helper back to the
    # identity function must reproduce the regression: the call that denies today flips
    # to allow.
    s = write_settings(tmp_path)
    command = (
        f"git commit -F - <<'EOF';(dd of={s} bs=1 count=0)\n"
    ...
    monkeypatch.setattr(hook.shell_tokens, "_split_punct_run", lambda tok: [tok])
    assert hook.decide(call) is None


def test_mutation_forcing_no_process_substitution_check_reddens_a_deny_case(tmp_path, monkeypatch):
    # Finding 6's own primitive, `shell_tokens.has_process_substitution`, exercised
    # through the CLEAN-lex command (no heredoc, no odd apostrophe) that reaches
    # `_bash_widening`'s main path and never touches `_write_incapable_bash_call` at
    # all -- so this pins the shared primitive at the one call site nothing else in
    # this suite reaches. Forcing it back to "never detected" must reproduce the
    # pre-fix regression: `bash_write_targets.command_write_targets` truncates its
    # operand scan at the first `<`/`>` token (`_operands_until_redirect`) and finds no
    # candidate at all for this segment, so the call that denies today flips to allow.
    s = write_settings(tmp_path)
    command = f"git commit -F - <(dd if=/tmp/evil-source of={s} bs=1)"
    call = payload("Bash", {"command": command}, ARMED_BASH, tmp_path)
    assert hook.decide(call) is not None

    monkeypatch.setattr(hook.shell_tokens, "has_process_substitution", lambda *_: False)
    assert hook.decide(call) is None


def test_mutation_forcing_no_newline_normalization_reddens_a_deny_case(tmp_path, monkeypatch):
    # Finding 7's own primitive, `shell_tokens.normalize_newline_separators` -- shared
    # by BOTH the general path (`bash_write_targets.command_write_targets`, which calls
    # it directly) and the fallback path (`shell_tokens.separator_exact_split`, which
    # calls it internally before lexing), exactly the dual-path discipline Finding 6
    # required. This exercises the GENERAL path alone: no heredoc, no odd apostrophe, so
    # `_write_incapable_bash_call` is never reached. Forcing the primitive back to the
    # identity function must reproduce the pre-fix regression on its own: the bare `\n`
    # between `echo hi` and `tee <surface>` again collapses into one `echo`-only segment
    # and the call that denies today flips to allow.
    s = write_settings(tmp_path)
    command = f"echo hi\ntee {s} </dev/null\n"
    call = payload("Bash", {"command": command}, ARMED_BASH, tmp_path)
    assert hook.decide(call) is not None

    monkeypatch.setattr(hook.shell_tokens, "normalize_newline_separators", lambda text: text)
    assert hook.decide(call) is None


def test_mutation_forcing_plain_shlex_in_segments_reddens_a_deny_case(tmp_path, monkeypatch):
    # `bash_write_targets.segments()`'s own primitive, Finding 8's round-10 fix: without
    # `separator_exact_split`, the MAIN clean-lex path reproduces the identical glued-
    # separator regression `test_mutation_forcing_plain_shlex_split_reddens_a_deny_case`
    # pins on the FALLBACK path above -- but here with no heredoc at all, so this is the
    # only control in the suite that kills a regression to plain `shlex.split` inside
    # `segments()` itself, the shared tokenizer `command_write_targets` and
    # `_unmodelled_writer_segment` both depend on.
    s = write_settings(tmp_path)
    command = f"echo hi;dd of={s} if=/dev/null"
    call = payload("Bash", {"command": command}, ARMED_BASH, tmp_path)
    assert hook.decide(call) is not None
...
def _commenting_separator_exact_split(text: str) -> list[str]:
    """`shell_tokens.separator_exact_split` MINUS its `lexer.commenters = ""` line.

    The assignment is local to that function, so the mutant cannot be produced by
    patching an attribute -- it is reconstructed here, keeping every other property of
    the real lexer (`posix`, `punctuation_chars`, `whitespace_split`, the newline
    normalization, the punctuation-run re-split) so the differential isolates exactly
    the one line under control.
    """
    lexer = shlex.shlex(
        hook.shell_tokens.normalize_newline_separators(text), posix=True, punctuation_chars=True
    )
    lexer.whitespace_split = True
    tokens: list[str] = []
    for tok in lexer:
        if tok and all(c in hook.shell_tokens._PUNCT_CHARS for c in tok):
            tokens.extend(hook.shell_tokens._split_punct_run(tok))
        else:
            tokens.append(tok)
    return tokens


def test_mutation_forcing_a_commenting_lexer_reddens_a_deny_case(tmp_path, monkeypatch):
    # `separator_exact_split`'s `lexer.commenters = ""`. `shlex.shlex` defaults
    # `commenters` to `'#'` where plain `shlex.split` sets it to `''`, and this lexer is
    # the first to run over the WHOLE multi-line command (bare newlines already rewritten
    # to `;`), so with the default left in place a `#` anywhere ENDS the token stream:
    # every statement on a later line vanishes from `segments()`, the command collapses
    # to one segment led by the write-incapable `git`, and a real writer behind the
    # comment is invisible to every consumer. Real bash runs the second statement
    # regardless of the comment and genuinely overwrites the settings file.
    s = write_settings(tmp_path)
    command = f"git status # look first\ndd of={s} if=/dev/zero bs=1 count=5"
    call = payload("Bash", {"command": command}, ARMED_BASH, tmp_path)
    assert hook.decide(call) is not None

    monkeypatch.setattr(
        hook.shell_tokens, "separator_exact_split", _commenting_separator_exact_split
    )
    assert hook.decide(call) is None


def test_mutation_forcing_env_write_incapable_reddens_a_deny_case(tmp_path, monkeypatch):
    # `_WRITE_INCAPABLE_WORDS`'s round-10 fix, the removal itself: without it, `env dd
    # of=<surface>` is judged write-incapable on `env` alone and `dd`'s own write-
    # capable `of=` idiom never reaches `_unmodelled_writer_segment`. Forcing the set
    # back to include `env` must reproduce the regression: the call that denies today
    # flips to allow.
    s = write_settings(tmp_path)
    command = f"env dd if=/dev/zero of={s} bs=1 count=5"
    call = payload("Bash", {"command": command}, ARMED_BASH, tmp_path)
    ...
    monkeypatch.setattr(hook, "_WRITE_INCAPABLE_WORDS", hook._WRITE_INCAPABLE_WORDS | {"env"})
    assert hook.decide(call) is None
