...
def _record_passed(store, sid, **kw):
    """Record stage 1 passed, whatever the machine verification then makes of it.

    A substantive stage's pass also needs an --observation (Defect 2). These tests
    are about WHERE the verify_command runs, so it is fixed here once: worded to
    stay true of the two cases below whose passed claim the verification overrides
    into a failure — what was observed is the exit status, not that it was zero.
    """
    return cli.cmd_record_result(
        ns(session=sid, status="passed", actual="ok", control=None,
           observation="ran the stage's verify_command in the recorded venue and read back its exit status"),
        store=store, **kw,
    )
...
def test_cd_prefix_applied_when_repo_root_set(store):
    ...
    _record_passed(store, "c1", runner=cap)
...
def test_no_prefix_when_repo_root_none(store):
    ...
    _record_passed(store, "c2", runner=cap)
