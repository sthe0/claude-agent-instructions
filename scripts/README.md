...
| [hook-thermometer-digest-due.py](hook-thermometer-digest-due.py) | SessionStart: 30-day throttled stderr nudge to run `thermometer-digest.py` and check the σ build-trigger (stamp `~/.local/state/claude-thermometer.stamp`); nudge only, does not auto-scan |
