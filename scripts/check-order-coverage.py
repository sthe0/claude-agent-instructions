...
Accepts exactly three control-name grammars — `controls.COVERAGE_GRAMMARS`,
matched by PREFIX (never exact equality) so an author may append a trailing
free-prose parenthetical after the fixed form — e.g. "stage 12 landed assertion
(full: containment in main and origin/main, plus reachability of the delivered
commit)":
...
The resolver itself lives in `agentctl/controls.py`, shared with the
question-materiality check, which admits two further grammars. That check's wider
set must never reach this caller: the grammar set is a parameter and this module
is the one place that binds it to the three above.
"""
...
def resolve_control(name: str, doc) -> str | None:
    ...
    return controls.resolve_control(name, doc, grammars=controls.COVERAGE_GRAMMARS)
