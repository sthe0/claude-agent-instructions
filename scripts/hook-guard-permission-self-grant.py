...
it is allowed without being judged, which is a named residual at `decide()`.
  ...
  Bash      — deliberately coarser. A command is not applied before it runs, so no
              ...
              surface, OR the command contains a segment this gate can neither place as
              write-incapable nor resolve to a target (`_unmodelled_writer_segment`) — an
              UNKNOWN, not a named target, but armed UNKNOWN still denies. `install SRC
              DIR` was outside both checks until round 7 (the verb was unmodelled, and the
              default posture then was silent ALLOW); `tar -xf a.tar -C DIR`, and anything
              routed through a script the hook cannot read (`bash /tmp/grant.sh`), are
              outside the first check but inside the second. The residual below names what
              still passes both.
...
NAMED RESIDUALS carried by this hook (R-numbers are the plan's):
  ...
  R7  The Bash path has no not-relevant branch — see above. It is bounded on two
      ...
      model it does not REPORT A CANDIDATE FOR. `_unmodelled_writer_segment` closes
      this axis AS A CLASS rather than verb by verb: a clean-lexing segment led by a
      program neither on `_WRITE_INCAPABLE_WORDS` nor already answered by
      `segment_write_target` (a redirect, or an enumerated writer verb) is UNKNOWN,
      not a silent fall-through — whatever the unenumerated verb is. This closes what
      were previously two separate residual members. WRITE VIA AN UNMODELLED VERB:
      `dd if=/dev/zero of=…`, `curl -o …`, `openssl … -out …`, `rsync`; `install` was
      in this class until round 7, which is the evidence the class is real and not
      theoretical — an ordinary write verb, simply absent from the table, that
      allowed a copy onto a live settings file. WRITE ROUTED THROUGH A SCRIPT OR
      INTERPRETER: `bash /tmp/grant.sh`, `python3 grant.py`, `./grant.sh` — the
      write itself is invisible to a gate that reads only the command line, but the
      LAUNCHING word is now itself an unmodelled verb (`bash`, `python3`, a bare
      script path — none are on `_WRITE_INCAPABLE_WORDS`), so the call denies via
      UNKNOWN even though the gate never learns what the script writes. `tar` falls
      in the first member for the same generic reason (`tar` is not on
      `_WRITE_INCAPABLE_WORDS` and is not an enumerated writer verb), so
      `tar -xf a.tar -C DIR` now denies too — WITHOUT this gate ever needing a second
      grammar to read `tar`'s own archive-relative destinations, because UNKNOWN does
      not require naming the right path, only refusing to call the call safe. Two
      members stay open, and are open for a different reason — not an unenumerated
      verb, but a resolved candidate that is wrong or absent by construction. (iii)
      The patch verbs, which DO report a target — the working directory — and are
      their own paragraph below. (iv) A MODELLED verb whose target the lexer resolves
      to THE WRONG PATH, which is the worse of the two because nothing about it looks
      like a gap: the verb is in the table, the
...
installed" and "is installed but denies wrongly" stay two distinguishable failures.
  ...
  MODELLED IS NOT REGISTERED, and the gap is an UNVERIFIED ASSUMPTION rather than a
  known defect. `_FILE_TOOLS` models four tools, but `install-reminder-hooks.sh`
  registers this hook under two matchers only, `Edit|Write` and `Bash`. That
  `MultiEdit` and `NotebookEdit` calls reach this hook at all therefore rests
  entirely on the harness treating a matcher as an UNANCHORED regex, so that
  `Edit|Write` matches the substring `Edit` inside both longer names. Nothing states
  that contract — not the installer, not `lib/hook_wiring.py`, not this file — and
  nothing in this unit probed it: the client bundle is outside the delivering
  session's readable scope, and no live `MultiEdit` was aimed at a scratch surface.
  If the assumption is false, both limbs are dead code and the gate under-fires on
  exactly the two tools whose modelling was already paid for. The same convention is
  reused unmodified by six other pre-existing hooks in this installer, so settling it
  is worth more than this gate alone. `live-run-evidence-permission-self-grant.md` § 8
  records the same gap from the firing-witness side ("Only the `Edit` limb is
  witnessed"); the two statements must move together.
...
# Command words that can neither write a file at an arbitrary path nor execute their
# stdin or another program. `git` earns its place only alongside the sub-allowlist below,
# since `git apply`/`git checkout -- <path>` both write. Shared by two call sites:
# conjunct (B) of `_write_incapable_bash_call` (the pre-lex-failure fallback), and
# `_unmodelled_writer_segment` (the clean-lex default posture on `_bash_widening`'s main
# path) — one membership list rather than two, so a word cleared here is cleared
# identically on both paths. `jq` and `cat` are in it for the second site: `jq . <file>`
# is the `_bash_write_target_rows` load-bearing ALLOW row, and neither `jq` nor `cat` has
# a flag that writes a file at an arbitrary path — only a shell redirect can, and that is
# caught separately (by `segment_write_target`'s own redirect scan) regardless of verb.
#
# THE REST OF THIS SET (`grep`…`more`) exists because `_unmodelled_writer_segment` makes
# this list's completeness load-bearing for the first time: before it, a verb missing
# here was silently ALLOWED (invisible), so the list only ever grew when a write leaked
# through; now a verb missing here is UNKNOWN while armed, so an incomplete list is felt
# immediately, by ordinary read-only commands — measured live, on `grep`, in the same
# session that built this fix. Every member below was checked against its actual option
# grammar for a flag or positional operand that names an output path (the shape this
# round is closing, not verb identity): plain stdin/stdout tools with no such flag. Two
# near-misses deliberately excluded: GNU `sort -o FILE` and `uniq IN [OUT]` both name an
# arbitrary output path directly, so both stay unmodelled and deny like `dd`. This list
# WILL still be missing some genuinely write-incapable verb — that is an accepted
# residual under this hook's own asymmetric cost (`_ON_ERROR`'s docstring): the failure
# mode is one blocked command a user sanctions in a sentence, never a silent allow.
#
# R9: `cd`/`pushd`/`popd` were missing entirely — a gap found the hard way, because the
# harness itself routinely prefixes an ordinary Bash call with `cd <dir> && ...`, so the
# omission denied nearly every Bash call once this default posture shipped, not just a
# rare shape. All three are ordinary shell builtins that manage the shell's working-
# directory state: none can write a file at an arbitrary path, and none executes another
# program on their own (a `&&`-chained command after one is its own separate segment,
# already judged independently). `export`/`unset`/`local` were considered and deliberately
# left OFF this round — same "leave a verb off when in doubt" discipline the rest of this
# list already documents — pending a full developer/reviewer pass rather than a
# speculative addition made under a Bash-usage outage.
_WRITE_INCAPABLE_WORDS = frozenset({
    "git", "echo", "printf", "jq", "cat",
    "grep", "egrep", "fgrep", "rg", "ls", "head", "tail", "wc", "tr", "cut",
    "pwd", "whoami", "date", "sleep", "true", "false", "which", "type",
    "printenv", "hostname", "uname", "id", "du", "df", "stat", "file",
    "less", "more", "cd", "pushd", "popd",
})
_GIT_READ_OR_RECORD_SUBCOMMANDS = frozenset(
    {"commit", "status", "log", "diff", "show", "add", "rev-parse"}
)

# R9: the `git` carve-out originally checked only `seg[1]` against
# `_GIT_READ_OR_RECORD_SUBCOMMANDS`, so any invocation carrying a leading global flag
# (`git -C <dir> status`, `git --no-pager log`) never matched — `seg[1]` was the flag, not
# the subcommand. These two tables enumerate the common git global flags so the walker
# below can skip past them to find the real subcommand. Flags that take a SEPARATE
# argument token are in `_GIT_GLOBAL_FLAGS_WITH_ARG`; flags that take none are in
# `_GIT_GLOBAL_FLAGS_NO_ARG`; a glued long-option form (`--git-dir=<path>`) is a single
# token, matched by prefix rather than consuming an extra one.
_GIT_GLOBAL_FLAGS_WITH_ARG = frozenset({"-C", "-c", "--git-dir", "--work-tree"})
_GIT_GLOBAL_FLAGS_NO_ARG = frozenset({
    "--no-pager", "-p", "--paginate", "--bare", "-P", "--no-optional-locks",
    "--literal-pathspecs",
})
_GIT_GLOBAL_FLAG_GLUED_PREFIXES = ("--git-dir=", "--work-tree=")


def _git_subcommand_after_global_flags(seg: list[str]) -> str | None:
    """Walk `seg` (assumed `seg[0] == "git"`) past a KNOWN, ENUMERATED set of git global
    flags — consuming a following argument for the ones that take one — to find the
    actual subcommand token. Returns None on any token this walker does not recognize, or
    if the walk runs off the end with no subcommand found: the caller then treats the
    segment as NOT write-incapable (denies), the same fail-toward-UNKNOWN discipline
    `_ON_ERROR`'s own docstring states for every other conjunct in this file — an
    unrecognized git global flag denies rather than being guessed safe."""
    i = 1
    n = len(seg)
    while i < n:
        tok = seg[i]
        if tok in _GIT_GLOBAL_FLAGS_WITH_ARG:
            i += 2
            continue
        if tok in _GIT_GLOBAL_FLAGS_NO_ARG:
            i += 1
            continue
        if tok.startswith(_GIT_GLOBAL_FLAG_GLUED_PREFIXES):
            i += 1
            continue
        return tok
    return None
...
def _segment_word_is_write_incapable(seg: list[str]) -> bool:
    """True iff `seg`'s leading command word is on `_WRITE_INCAPABLE_WORDS`, honoring the
    `git` carve-out (a bare `git` word is write-incapable only when the subcommand found
    by walking past any leading global flags, via `_git_subcommand_after_global_flags`, is
    a read-or-record subcommand). Factored out so conjunct (B) below and
    `_unmodelled_writer_segment` read the identical definition rather than two that could
    drift."""
    word = os.path.basename(seg[0])
    if word not in _WRITE_INCAPABLE_WORDS:
        return False
    if word == "git":
        subcommand = _git_subcommand_after_global_flags(seg)
        return subcommand is not None and subcommand in _GIT_READ_OR_RECORD_SUBCOMMANDS
    return True

# Conjunct (C)'s backstop: the writer verbs `bash_write_targets.segment_write_target`
# itself recognizes, checked over the WHOLE raw command text rather than a parse, because
# this conjunct exists precisely for the text a parse could not reach (the heredoc body).
_WRITER_VERBS = frozenset({"patch", "tee", "sed", "cp", "mv", "install", "apply"})
_RAW_TEXT_WORD_RE = re.compile(r"[A-Za-z0-9_./-]+")
...
def _write_incapable_bash_call(command: str) -> bool:
    """True iff `command` cannot write a file even though its FULL text (heredoc bodies
    included) fails to tokenize — the case `_bash_widening` reaches only on the pre-lex
    failure path, once the body-stripped lex has already raised.

    Seven conjuncts, each individually insufficient:

    (A) The command line PROPER — up to the first newline not preceded by a line
        continuation, i.e. before any here-document body — lexes on its own, via
        `shell_tokens.separator_exact_split` rather than plain `shlex.split`. If it does
        not, the unparseability lives in command text rather than a body, and this
        returns False so the caller falls back to `_Unknown` exactly as before this
        function existed: that is what keeps the fail-closed invariant honest.
    (B) Every pipeline/list segment's command word on that line is write-incapable
        (`_WRITE_INCAPABLE_WORDS`), and any `git` word is immediately followed by a
        read-or-record subcommand (`_GIT_READ_OR_RECORD_SUBCOMMANDS`) — `git apply` and
        `git checkout -- <path>` both write, so a bare `git` does not qualify alone.
        Segmentation here MUST be separator-exact — `bash_write_targets.split_segments`'s
        own docstring accepts a glued separator (`a;b` left as one token) as a residual
        tolerable for ITS original callers, who only need some segment to answer, but
        (B) reads "every segment's word is write-incapable" as a claim over the WHOLE
        line, so a glued `;dd` or `|bash` would hide a second, write-CAPABLE command's
        real word entirely (`echo hi;dd of=<surface>`; `echo hi|bash <<'EOF'` piping a
        heredoc body into `bash` as code). `separator_exact_split` closes that by
        construction — a punctuation-aware re-tokenization on which `;`/`|`/`||`/`&`/
        `&&`/`|&` are always their own token — rather than by teaching this conjunct to
        pattern-match the glued shape.
    (C) The FIRST heredoc/here-string operator on that command line, if any, accounts
        for the ENTIRE rest of `command` — `shell_tokens.first_heredoc_consumes_entire_
        tail`. Without this, a second command after the first heredoc's terminator line
        (invisible to (A)/(B), which only ever look at the command line proper) can be
        an arbitrary write the writer-verb table in (E) does not even get a chance to
        catch if it is spelled with shell quoting an unlexed textual scan cannot resolve
        (`printf x <<'EOF'\n...\nEOF\nt''ee <surface>` — bash really runs a second, fully
        independent command there). Reducing the question to "does the tail hold ONLY a
        single body" is what lets (E)'s textual scan over that body stay sound despite
        never lexing it: there is nothing else in the tail left for that scan to miss.
    (D) The WHOLE raw command text — every here-document body included — contains no
        `>`.
    (E) No word from the writer-verb table (`_WRITER_VERBS`, the same verbs
        `bash_write_targets.segment_write_target` itself treats as writers) appears
        anywhere in that text either. Textual, not a parse: strictly more conservative
        than what a clean lex would see, and it is what stops (A)+(B) from reading a
        body no parse ever looked at. Safe specifically because of (C): the tail this
        scans is never anything but the first heredoc's own body, never a second
        command whose quoting could hide a writer verb from a textual scan.
    (F) The body itself is one the SHELL will not expand before the write-incapable
        program in (B) ever starts — `shell_tokens.first_heredoc_body_shell_inert`,
        mirroring `shell_tokens.py`'s own clause (vi): a quoted delimiter (`<<'EOF'`,
        `<<"EOF"`), or a body free of `$`, backtick and backslash. Conjuncts (A)-(E) all
        answer questions about the COMMAND; this is the one that answers a question
        about the BODY, because an unquoted delimiter (`<<EOF`, `<<-EOF`) lets bash
        expand `$(...)` inside the body itself, before `git` (or any other
        write-incapable word) ever runs — so `git commit -F - <<EOF` with
        `$(touch /tmp/pwned)` in its body genuinely writes, however write-incapable
        `git commit` is. Skipping this was a real defect here, caught in review before
        this landed: (B)+(D)+(E) alone let exactly that call through.
    (G) No process-substitution opening sequence — `shell_tokens.
        has_process_substitution`, i.e. no `<(` or `>(` — appears anywhere in the WHOLE
        raw command text. `<(cmd)`/`>(cmd)` forks and runs `cmd` as a side effect of
        preparing the outer command's argument list, and neither `<` nor `>` is a bash
        statement/pipe separator — so a nested writer rides along as a trailing token of
        the SAME segment as the write-incapable word (B) already approved, invisible to
        every conjunct above: `git commit -F - <(dd if=<attacker-file> of=<surface>
        bs=1) <<'EOF'` passes (A)-(F) exactly as this docstring's own repro history
        shows, because (D)/(E) scan the raw text for `>`/writer-verb WORDS and `dd`
        with `if=`/`of=` operands is neither. Textual, same conservative philosophy as
        (E): true whenever the two characters appear anywhere, including inside a
        segment (B) already accepted, so it has to run independent of (B)'s per-segment
        verdict rather than as a refinement of it. `_bash_widening` also runs this same
        check on its OWN entry, ahead of even attempting the pre-lex this
        function exists to substitute for — so in practice that earlier check is what
        denies a call before it ever reaches this function. (G) is this function's own
        half of the same contract: whatever calls `_write_incapable_bash_call` directly
        must not have it answer True for text a real shell would fork a nested command
        out of.

    Named residual: a heredoc body merely mentioning `>` or a writer-verb word (an
    ordinary commit message discussing `tee` or shell redirection) still denies. That is
    narrow and honest, and the deny message already tells the caller to commit via
    `git commit -F <file>` instead. Widening (E) to distinguish body prose from a command
    is exactly the second grammar this module's docstring refuses to invent.
    """
    try:
        tokens = shell_tokens.separator_exact_split(shell_tokens.command_line(command))  # (A)
    except Exception:
        return False
    for seg in bash_write_targets.split_segments(tokens):  # (B)
        if not seg:
            continue
        if not _segment_word_is_write_incapable(seg):
            return False
    if not shell_tokens.first_heredoc_consumes_entire_tail(command):  # (C)
        return False
    if ">" in command:  # (D)
        return False
    if any(
        os.path.basename(token) in _WRITER_VERBS
        for token in _RAW_TEXT_WORD_RE.findall(command)
    ):  # (E)
        return False
    if shell_tokens.has_process_substitution(command):  # (G)
        return False
    return shell_tokens.first_heredoc_body_shell_inert(command) is not False  # (F)


def _unmodelled_writer_segment(command: str, cwd: str) -> str | None:
    """The leading word of the first segment of `command` this gate can neither clear as
    write-incapable NOR already resolves a real write-target candidate for, or None if
    every segment is accounted for one way or the other.

    THE ROOT THIS ANSWERS, not a per-program patch. `bash_write_targets.command_write_
    targets` is a verb ENUMERATION: `patch`/`git apply`/`sed -i`/`tee`/`cp`/`mv`/`install`,
    plus a bare shell redirect that any verb can carry. A verb outside that enumeration —
    `dd`, `curl -o`, `openssl … -out`, `sqlite3 <file> "..."`, `xxd -r`, `base64 -d -o`, or
    any future writer nobody has enumerated yet — reports NO candidate for its segment,
    byte-identical to a segment that genuinely writes nothing (`jq . file`, `git status`).
    `_bash_widening`'s main loop over `command_write_targets` cannot tell those apart, so
    trusting an empty overall candidate list as "this command writes nothing" is exactly
    the R7-axis-1(ii) hole: an unreported target reaching ALLOW by a route that never
    reached a verdict. Enumerating more writer verbs narrows the hole one program at a
    time and never closes it — `dd` today, `curl`/`openssl`/`sqlite3` tomorrow, forever
    one step behind whatever a caller reaches for next.

    THE FIX FLIPS THE DEFAULT FOR THE UNENUMERATED CASE, rather than growing the
    enumeration. A segment is safe to skip only two ways: it is write-incapable BY
    DESIGN — `_segment_word_is_write_incapable`, the same definition conjunct (B) of
    `_write_incapable_bash_call` uses on its own fallback path — or `bash_write_targets.
    segment_write_target` already resolved a real candidate for it (a shell redirect, or
    one of the enumerated writer verbs), which `_bash_widening`'s own loop then judges on
    its merits. Everything else — a program this gate does not recognize as either — is
    UNKNOWN rather than a silent ALLOW; while armed that costs one blocked call, which a
    user sanctions in a sentence, exactly the asymmetry `_ON_ERROR`'s own docstring states.

    Deliberately independent of whether the segment ALSO carries a redirect: a redirect
    target is resolved and judged by the caller's own loop over `command_write_targets`
    regardless of this function's verdict, so flagging the verb here too is redundant with,
    never a substitute for, that judgement — and harmless, since a segment that already
    produced a candidate is skipped here (see above) whether or not that candidate turned
    out to be a surface."""
    for seg in bash_write_targets.segments(command):
        if not seg or bash_write_targets.segment_write_target(seg, cwd):
            continue
        if _segment_word_is_write_incapable(seg):
            continue
        return os.path.basename(seg[0])
    return None
...
def _bash_widening(tool_input: dict, cwd: str) -> _Widening | _Unknown | None:
    ...
    2). The pre-lex below calls `shell_tokens.separator_exact_split` over exactly the
    text `bash_write_targets.segments` lexes — `strip_heredoc_bodies(command)` — i.e.
    the SAME function over the SAME text, so the two agree on what "could not parse"
    means BY CONSTRUCTION rather than by two lexers being similar enough. Plain
    `shlex.split` was measurably not that: it is a different object with a different
    failure set (`shlex.shlex`'s own `commenters` default among them), so a command
    could pass the guard here and then fail inside `segments`, where the failure is
    fail-open (`[]`) and reads as "no write target" — an ALLOW by a route that never
    formed a verdict. The agreement holds UNLESS
    `_write_incapable_bash_call` shows the failure lives entirely inside a body that
    cannot be turned into a write regardless: settling the write question on the
    command WORD is what makes that tokenization unnecessary in the first place.
    ...
    AN EMPTY CANDIDATE LIST IS NOT TRUSTED EITHER, for the same reason a pre-lex failure
    is not: `command_write_targets`'s verb table is an enumeration, so a segment led by a
    program outside it (`dd`, `curl -o`, `openssl … -out`, `sqlite3`, …) reports no
    candidate at all — indistinguishable, by truthiness, from a segment that genuinely
    writes nothing (`jq .`, `git status`). Once the loop above finds no definite widening,
    `_unmodelled_writer_segment` is what tells the two apart before this function trusts
    the silence as "writes no surface": see its own docstring for the full account.

    A PROCESS-SUBSTITUTION CHECK RUNS BEFORE EITHER BRANCH, deliberately outside both
    the pre-lex try/except and the `command_write_targets` loop it guards. `<(cmd)`/
    `>(cmd)` forks and runs `cmd` regardless of whether the outer command ever succeeds,
    and neither `<` nor `>` is a bash separator, so a nested writer never starts its own
    segment for `bash_write_targets.split_segments` to see — it rides along as a
    trailing token nothing downstream inspects, WHETHER OR NOT the surrounding text
    happens to tokenize cleanly. A directory-shaped candidate (the existing `patch`/
    `git apply` convention `command_write_targets` already reports) cannot substitute
    for this: this gate reads a directory as "not a permission surface", i.e. ALLOW, so
    the check has to deny outright rather than ride the candidate list.
    """
    ...
    stripped = shell_tokens.strip_heredoc_bodies(command)
    if shell_tokens.has_process_substitution(stripped):
        return _Unknown(
            "its command text contains a process substitution (`<(...)`/`>(...)`), which "
            "forks and runs a nested command invisible to this gate's write-target "
            "resolution, so whether it writes a permission surface is unknown rather "
            "than known to be no"
        )

    try:
        shell_tokens.separator_exact_split(stripped)
    except Exception:
        if _write_incapable_bash_call(command):
            return None
        return _Unknown(
    ...
    if unknown is not None:
        return unknown
    word = _unmodelled_writer_segment(command, cwd)
    if word is not None:
        return _Unknown(
            f"its command contains a segment led by `{word}`, a program this gate does "
            f"not recognize as either write-incapable or a modelled writer, so whether "
            f"it writes a permission surface is unknown rather than known to be no"
        )
    return None
