...
def main(argv: list[str] | None = None) -> int:
    ...
    # Cross-host refusal (light, mirrors spawn-cursor-escape.py): this wrapper
    # only ever shells out to `claude` (the cmd list built below). A session
    # sticky-bound to runtime_host=cursor (agentctl.runtime_host) must never
    # reach this Claude spawn wrapper — refuse explicitly rather than silently
    # spawning the wrong CLI family.
