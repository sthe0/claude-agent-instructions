...
def main(argv: list[str] | None = None) -> int:
    ...
    # Cross-host refusal: this wrapper only ever shells out to `agent`/`cursor-agent`
    # (build_agent_cmd below). A session sticky-bound to runtime_host=claude
    # (agentctl.runtime_host) must never reach a Cursor spawn wrapper — refuse
    # explicitly here rather than let it silently spawn the wrong CLI family.
