...
def spawn_cli_for(host: str) -> Path:
    """The wrapper script `runtime_host=host` dispatches through: spawn-specialist.py
    (`claude -p`) for HOST_CLAUDE, spawn-cursor-specialist.py (`agent -p`) for
    HOST_CURSOR. This is the ONE seam that decides which binary family a real
    dispatch ends up shelling out to — build_argv only ever spells out this
    script's path, so a runtime_host=cursor dispatch can never assemble a `claude`
    invocation (or vice versa) BY CONSTRUCTION, with no separate runtime binary
    preflight needed on this path (unlike lib.host_llm's judge/extractor argv,
    which builds a bare `<binary> -p ...` call directly and so DOES need
    host_llm.assert_same_family as a runtime guard)."""
    try:
