...
Resolve each return marker, then re-spawn with the resolution in a continuation prompt. **`PLAN-READY:` is a hard gate** — stop and get explicit user approval before any further spawn; never infer approval from silence. `CLARIFY:` — answer the one fact (ask the user first if needed). `REPLAN:` — incorporate the revision. `PERMISSION-REQUEST:` — check existing grants via `scripts/permissions-cli.py check` (global **and** project) before asking; persist an `always` grant via `permissions-cli.py grant`. `ESCALATE:` — resolve the decision (with the user if needed). `INCOMPLETE:` — re-spawn / ask / accept partial. `COMPLETED:` — next step. Per-marker continuation-prompt templates and the workflow-vs-tool-permission distinction: [handling-escalations.md](memory-global/leaves/handling-escalations.md).
