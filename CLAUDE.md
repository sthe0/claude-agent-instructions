...
- **Push, then land — proactively, at the resolution gate.** *Difficulty: `resolve` records task-done but has no VCS-integration beat, so committed work strands — first **locally** (unpushed: no remote backup, defeating the "push after each commit" safety net), then as a branch left unmerged on its personal remote ("подвисла ветка").* **Striving to deliver committed work to its terminal VCS state is the default, not an afterthought — never a passive "tell me if you want to push".** When the resolved task committed to a personal working branch, **bundle the VCS-integration action into the resolution `AskUserQuestion`** with the delivering option **first and `(Recommended)`**: **(a)** if the branch is unpushed / ahead of its upstream, offer **push** — pushing a personal / working branch is pre-authorized (§ Acting without asking #4), so this is the recommended default; **(b)** if it is cleanly fast-forwardable into trunk, additionally offer **land+delete** (trunk-push needs explicit confirmation, so it belongs in that same click-gate). On a land confirm, use `scripts/land-branch.py` (`--check` previews; it does ref-only `git push <remote> <branch>:<trunk>` ff + `push --delete` + `git branch -f`, refusing non-ff) — never hand-roll `checkout` / `reset --hard` / `clean` on a shared tree, and leave any parallel-session uncommitted WIP untouched. `hook-resolution-reminder.py` surfaces this at the gate when a landable **or unpushed** branch exists.
