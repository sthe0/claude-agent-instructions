...
- **Land the working branch.** *Difficulty: `resolve` records task-done but has no VCS-integration beat, so confirmed-resolved branches pile up unmerged on their personal remote ("подвисла ветка").* When the resolved task rode a personal working branch now cleanly fast-forwardable into trunk, **bundle** a land+delete option into the resolution `AskUserQuestion` (trunk-push needs explicit confirmation, so it belongs in that same click-gate). On confirm, land via `scripts/land-branch.py` (`--check` previews; it does ref-only `git push <remote> <branch>:<trunk>` ff + `push --delete` + `git branch -f`, refusing non-ff) — never hand-roll `checkout` / `reset --hard` / `clean` on a shared tree, and leave any parallel-session uncommitted WIP untouched. `hook-resolution-reminder.py` surfaces this at the gate when a landable branch exists.
