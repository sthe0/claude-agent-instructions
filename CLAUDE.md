...
- **Gate not passed ⇒ no close.** Verification failed → `overcome-difficulty`, not "close". **In-thread carve-out:** for in-thread work without a formal plan, the final-summary moment **is** the gate — put the `AskUserQuestion` in the **same reply** as the recap. **Time-driven symptoms** (tick, cron): not passed until ≥1 full period elapses post-fix with no recurrence — the silent interval is the observable, not a config edit or log line.
