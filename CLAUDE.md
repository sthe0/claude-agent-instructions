...
**Own research precedes escalation — at every moment a question arises, not only at plan approval.** Before a question reaches the user, spend the cheap channels you already hold: memory, the repo, docs/web search, an MCP, and any **domain-expert subagent this project defines** — that subagent is a capability you hold, so skipping it is [capability-before-offload](memory-global/leaves/capability-before-offload.md) on the *question* axis. The engine already codes this rule (`question-dispose --to escalated` refuses on empty `own_research`; authority `premise.validate_questions`), but it binds only questions recorded in the `premise` bag and only at the `plan_approval` gate — a question arising **mid-execution, or at plan review**, is bound by nothing but you, so run `question-raise` → `question-research` → `question-dispose` there too. *Difficulty:* a question you could have answered yourself spends the user's attention, the scarcest term in the objective function. Scope it by kind: a **knowledge** question is what a held channel closes; a decision, permission or preference is the user's to give and carries no research precondition.
...
**Two re-entry signals warrant a direct user question instead of another replan.** A second effort-divergence firing on the `replans` scale for the same order — the trigger re-arms once a replan itself produces another overrun measured from the new baseline — and any replan or review cycle re-entered on a session whose engine state is already `RESOLVED` both fall here. In both cases, ask via `AskUserQuestion` whether the order still warrants continuing, not a further replan; the trigger has already been answered once and the plan's current baseline has moved. This is a **prose-only stopgap** ahead of a structural round-release valve for the `replans` scale (the mechanized valve for the neighboring enumeration axis lives in a sibling plan); the structural gap is documented in [effort-divergence-trigger.md](memory-global/leaves/effort-divergence-trigger.md) § The honest weakness.
