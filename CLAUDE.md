...
Resolve each return marker, then re-spawn with the resolution in a continuation prompt. **`PLAN-READY:` is a hard gate** — stop and get explicit user approval before any further spawn; never infer approval from silence. `CLARIFY:` — answer the one fact (ask the user first if needed). `REPLAN:` — incorporate the revision. `PERMISSION-REQUEST:` — check existing grants via `scripts/permissions-cli.py check` (global **and** project) before asking; persist an `always` grant via `permissions-cli.py grant`. `ESCALATE:` — resolve the decision (with the user if needed). `INCOMPLETE:` — re-spawn / ask / accept partial. `COMPLETED:` — **diff the delivery against the user's approved intent (not just "tests pass") before the next step; a delivery that reinterprets an approved requirement is a substantive deviation needing re-approval (leaf).** Per-marker continuation-prompt templates and the workflow-vs-tool-permission distinction: [handling-escalations.md](memory-global/leaves/handling-escalations.md).

### On task resolution (record experience)

A substantive task is **resolved** only when the user explicitly confirms it — **don't wait for gratitude, close the loop proactively.** Once all stages pass their `Expected result image:` check and the plan's `## Final verification` passes against the overall done criterion, you are at the resolution gate.

**Closing protocol** (runs at the resolution gate):

1. **Verify done.** All stages green per `Expected result image:`? `## Final verification` passed? If no — `overcome-difficulty`, not "close". **In-thread carve-out:** for in-thread work without a formal plan (chain of small changes), the moment you're about to write the final summary **is** the gate — put the `AskUserQuestion` in the **same reply** as the recap, don't leave it for the user to prompt. **Time-driven symptoms** (periodic tick, cron): the gate isn't passed until ≥1 full period has elapsed after the fix with no recurrence — a config edit or log line is *not* the observable, only the silent elapsed interval is; verify via timestamps (no occurrences since the fix) or wait one period before step 3.
2. **Recap one line.** `Requested: <user's ask>. Delivered: <what was actually shipped>.` Keep it terse — one line each side.
3. **Ask explicitly via `AskUserQuestion`** in the user's language (mandatory per § Escalation to the user). **Shape the question to the criterion type:** *measurable* — generic "Считаем решённой?" (you've run the check); *acceptance-review* — name the **specific observation the user just performed** ("Запустил X — увидел Y?"), not a meta-belief, else "yes" can mean "the explanation sounded right" and a regression slips through (see `2026-05-25-resolution-gate-confirm-before-record.md`). **Bundle** the resolution ask with any other binary asks queued this turn (push, scope, follow-up) into one `AskUserQuestion` call.
4. **Wait for explicit confirmation.** An unambiguous "yes" / "resolved" / "так и оставим" / direct answer. **Bare gratitude is not confirmation** — `thanks` / `спасибо` / `perfect` alone is ambiguous between "thanks for the work" and "task is over"; ask anyway. Enforced by `scripts/hook-resolution-reminder.py` (UserPromptSubmit stderr nudge on brief-gratitude replies).
