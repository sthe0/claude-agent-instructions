...
- **Gate not passed ⇒ no close.** Verification failed → `overcome-difficulty`, not "close". **In-thread carve-out:** for in-thread work without a formal plan, the final-summary moment **is** the gate — apply the same split as [ask-user-question-split-turn.md](memory-global/leaves/ask-user-question-split-turn.md): recap as the turn's final message, arm the timer, ask next turn with the digest inside the question text. **Time-driven symptoms** (tick, cron): not passed until ≥1 full period elapses post-fix with no recurrence — the silent interval is the observable, not a config edit or log line.
