...
**Two re-entry signals warrant a direct user question instead of another replan.** A second `replans`-scale firing for the same order, and any replan/review cycle re-entered on an already-`RESOLVED` session: ask via `AskUserQuestion` whether the order still warrants continuing, not a further replan. Prose-only stopgap; structural gap and rationale: [effort-divergence-trigger.md](memory-global/leaves/effort-divergence-trigger.md) § The honest weakness.
