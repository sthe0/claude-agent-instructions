...
| `publish_plan` (on `submit_plan`) | Post the plan as a comment **before** asking for approval, then `plugin-record --phase plan`. The user reviews the plan on the ticket as well as in chat. Append the `agent-plan-sync` marker line from `python3 scripts/verify-ticket-plan-sync.py --emit-marker --plan <toml>` to the comment — it binds this exact plan version so a later continuation can detect drift. |
