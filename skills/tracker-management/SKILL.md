...
| `publish_plan` (on `submit_plan`) | Post the plan as a comment **before** asking for approval, then `plugin-record --phase plan`. The user reviews the plan on the ticket as well as in chat. The comment must carry the `agent-plan-sync` marker binding this exact plan version, so a later continuation can detect drift. When the resolved backend's `tracker_publish_plan` already auto-appends it (GitHub does, since it computes and appends the marker from `verify-ticket-plan-sync.py --emit-marker` internally), no extra step is needed. For any other backend, append the marker line yourself via `python3 scripts/verify-ticket-plan-sync.py --emit-marker --plan <toml>` before posting. |
...
**Resume across sessions.** Plugin state is per-session: a new session continuing an in-flight ticket starts with no tracker bag. Re-run Responsibility 0, then reconcile the ticket's last comment against the **actual current state** — the branch's commit log against trunk, PR status, prod runs, populated tables — and backfill a catch-up status comment for every meaningful boundary crossed since the last post. The across-session gap is exactly where progress posts silently vanish. Also run `python3 scripts/verify-ticket-plan-sync.py --plan <current toml> --comment-file <source>` against the ticket's last-posted plan comment. When the resolved backend defines the optional `tracker_plan_marker` verb (probe with `declare -F tracker_plan_marker`), fetch `<source>` directly: `<backend-call> tracker_plan_marker <key> | python3 scripts/verify-ticket-plan-sync.py --plan <current toml> --comment-file -` — no manual comment lookup needed. Otherwise (verb absent, or backend errors), fall back to manually locating the ticket's last plan comment and passing it as `--comment-file <path>` (the continuation-time `hook-ticket-plan-sync.py` nudges this check either way). **DRIFT** means the TOML was edited since posting — re-post the plan with a fresh `--emit-marker` marker (this is a plan actualization, not silent divergence). **NO-PLAN** means no plan was ever attached, or the marker predates this mechanism — confirm with the user before continuing rather than assuming an unplanned ticket is fine to keep working on.

**No engine plugin active? Post proactively anyway.** The phase-hooks table above assumes the `tracker` plugin is active (Responsibility 0) and lets the engine tell you *when*. A session that never routes through the full plan-approval spine — a chat-classified continuation, an ad-hoc monitoring loop — never activates that plugin, so no `publish_*` directive ever fires, and a genuine finding can sit unrecorded until the user asks whether it was. That is not license to stay silent: the moment you confirm a durable, non-obvious technical finding (a root-caused bug, a working config/API recipe, a resolved blocker) while working a ticket-linked task, write it to project memory immediately (already gate-exempt) **and** post a short status comment to the ticket — without waiting to be asked. Treat this as pre-authorized routine progress on an already-open, actively-worked ticket, not a new external action needing confirmation. Activating the plugin later (Responsibility 0) layers the engine's own reminder cadence on top of this default; it is not the precondition for it.
...
- Not tied to a specific tracker product. Jira / Linear / GitHub Issues (and any in-house tracker) specifics are project memory.
...
**The `present-plan` essence/full rendering is chat-facing, not ticket-facing — never repost it verbatim.** Its mandated first line is the plan's local absolute path (a convention for the manager-user dialogue, where the coordinator has filesystem access); reusing that same text as the `publish_plan` (or a `publish_result`) ticket comment leaks that local path straight into a public ticket, and the same collision shows up as a verify-command's local path dressed up as a markdown link. Deleting only the leading path line and reposting the rest unedited is not enough — write a short, ticket-audience paragraph instead, adapted for a reader who has no filesystem access and never saw the internal rendering.
...
**Open the structured record with a plain-language lead.** The Difficulty/Order & criterion/Context/Working-plan sections are the record's *body*, not its opening — before the first section, write one or two plain sentences stating the practical bottom line a reader wants first (what was actually wrong, what changed, what the reader gets out of it), per [tech-writer](../specializations/tech-writer/SKILL.md) § What to say, and in what order, rule 1 ("Exposition order: base → particular" — lead with one honest thesis, not a wall of terms). A tech-writer pass over the template's prose does not add this lead on its own — it polishes lexicon inside the section structure this skill fixes, it does not reorder that structure — so the lead is this skill's own requirement, not something to assume the polish pass supplies. See [artifact-complaint-dual-signal.md](../../memory-global/leaves/artifact-complaint-dual-signal.md) for the complaint this addresses.
