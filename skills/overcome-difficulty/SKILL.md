...
**A factor recurring across normalize cycles outranks a fresh one.** Before naming `--factor`, check it against factors already normalized earlier in this same task (declare/critique/normalize history, or the conversation if the engine wasn't used for earlier cycles). If the new factor belongs to the same underlying category as an earlier one — not the identical instance, but the same coupling or mechanism (e.g. two different defects both caused by shared mutable state crossing a test/production boundary) — the earlier normalize patched an instance, not the cause: whatever keeps reproducing it is still there. Recording another instance-level factor and moving on repeats that mistake. The replanning task must instead name the **structural alternative that removes the coupling itself** (not a further guard around it), and put it to the user as an explicit, named option alongside "patch again" — never folded into a continuation framing that omits it, and never skipped merely because the user authorized "one more round." Precedent and full rationale: [[recurring-normalize-factor-is-architecture-signal]].
...
The replanning task you just wrote is now the input for the root coordinator (you, in the main thread). **Before spawning the thinker plan-review, run `agentctl check-coverage --session <id> --new <corrected-plan>`** — a read-only pre-flight against the same coverage gate `replan` enforces, so a coverage miss surfaces before paying for a review that would only bounce off `replan` afterward; fix the plan and re-check until it reports `OK` before spending the spawn. **Before calling `agentctl replan`, a thinker review bound to the corrected plan is mandatory for every replan — refinement or substantive — but it is scope-aware, not always whole-plan.** Run `agentctl plan-review-delta --session <id> --new <corrected-plan>` first: via the same `changed_parts` coverage diff the gate itself checks, it reports whether the correction touched `meta`/`order` (whole-plan review required) or only specific stages (`--scope stage:<n>` suffices) — this is what keeps a one-stage fix from paying for a whole-plan re-review. Spawn `Task → thinker` with the critique's causal reasoning (functional ground, invariants-to-preserve, differences-to-remove) and either the full corrected plan or just the stage(s) `plan-review-delta` named, to check both the critique's reasoning and the corrected slice's adequacy/internal-consistency. The reviewer must compute the sha256 of the corrected plan file from its OWN read and pass it as `--plan-digest <sha256-hex>`; record the verdict with `agentctl plan-review --verdict pass|revise|override --reviewer thinker --target <corrected-plan> --scope <whole|stage:n,...> --plan-digest <sha256-hex> [--concern …]… [--note …]` — `replan` is engine-blocked (`gates.plan_review_blockers`) until a review covering every changed part exists bound to the exact corrected-plan path, and a `pass` does NOT bind without a `--plan-digest` matching the live plan bytes (a reviewer that could not read the plan cannot attest); only an explicit user-authored `override` bypasses a `revise` verdict (override needs no digest).
