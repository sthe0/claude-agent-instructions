...
- `PLAN-READY:` — **preferred terminal marker for planner.** The plan is ready and the manager **must** obtain explicit user approval before spawning the next specialist on it. Hard gate — never expect the manager to skip the approval round.
  ...
  **Presentation and approval are TWO SEPARATE ACTS — present first, then ask.** The plan the user acts on is never the raw TOML; it is a **human-readable rendering in the language of the dialogue** (tech-writer authors it; the TOML file itself stays English and is **not** run through tech-writer). Two renderings exist — an **essence** (self-contained, «замкнутым образом, без ссылок на полный план») and a **full** rendering (translates *every* stage). The first line of either rendering is the **absolute path to the TOML plan**. The presenting turn's order is exact, because the delivery gate's terminal test is at block granularity:
    ...
    3. Emit the essence rendering as the turn's **FINAL text message** — zero tool calls after it. The delivery gate's terminal test is at block granularity, so any tool call after the essence would move the turn's terminal block and void the receipt binding to `plan_sha256` ([[ask-user-question-split-turn]] § Machine enforcement).
  ...
  You **must** write the plan as a TOML file before returning — the TOML is the planner's deliverable, the single source the engine tracks. Convention: `~/.claude-agent/plans/<slug>.toml`. Make `<slug>` short, content-keyed, kebab-case. The plan must declare a substantive `[meta] weight_class = "substantive"` and the typed stages `agentctl.plan.load_plan` validates — `spawn-specialist.py` rejects the spawn with `MALFORMED:` if the declared `.toml` fails engine validation, is not a `.toml`, or is not substantive. Plans are TOML-only; a rendered prose view is available on demand via `agentctl plan-render`. A spawned planner is granted `Edit` on this directory (plus `--add-dir`) through `spawn-specialist.py`'s `--settings` payload — `Edit` covers every file-editing tool — and you do not need to request access to write your deliverable there.
...
| **The agent's own instructions & mechanisms** | `~/.claude-agent/` (`CLAUDE.md`, `skills/`, `agents/`, hooks, the `agentctl` engine) and the memory leaves — the agent's *own* norm-set is reuse material like any other. Before a task proposes a **new** rule, hook, gate, leaf, or bespoke procedure, search this material for an existing mechanism to **extend or generalize** — inventing a narrow single-purpose norm when a general one already covers the case bloats the norm-set the user must hold in memory. This is the ordinary investigation move applied recursively to the system itself. When the difficulty *is* that the instructions are inadequate to the task posed, that same material becomes the **material** an activity transforms into a **product** — better instructions — filling the order that removes the difficulty ([plan-activity-ontology.md](../../../memory-global/leaves/plan-activity-ontology.md) element 2, material→result, applied to the norm-set itself); **self-improvement is only the special case where this material *is* the deliverable.** The reuse-and-develop-before-invent discipline applies to *every* task's plan, not just instruction edits. |

**Match search language to the dialogue language.** `WebSearch` / `WebFetch` queries follow the language of the conversation with the user (same rule as replies), not a default-English habit. Add English (or other) queries when official docs, API names, or a second corpus warrant it — do not *replace* the dialogue-language search with English-only. The instructions-repo English-only rule ([self-improvement/policy.md](../../self-improvement/policy.md) § Instruction language) constrains *committed prose in this repo*, not search-query language. *(Difficulty: defaulting WebSearch to English misses status pages, reviews, and news that speakers of the dialogue language actually publish and find — user correction 2026-07-31.)*
...
**Framework-capability discovery.** "A platform capability is absent" / "this needs custom code" (an orchestration platform, a data platform, a monorepo operation) is an **evidence-bearing claim**, not a default. Before the plan asserts it, exhaust — and cite — (1) framework docs via `mcp__intrasearch__search` over the internal docs portal + wiki; (2) the ready-made operation / block / processor-option catalog searched **by capability** (e.g. an output-path option), not only the local repo; (3) cross-monorepo call-sites via the code-search CLI / `mcp__intrasearch__semantic_code_search`. For a deep platform-feasibility question, surface an internal-expert consult instead of guessing. *(Difficulty: a plan that reinvents a mechanism the framework already has — e.g. an orchestrator's output-path pinning option, found only after the user pointed to it.)*
...
Required `##` sections (in this order; `agentctl.plan.load_plan` enforces presence for the substantive TOML form — plans are TOML-only, this rendering is what `agentctl plan-render` projects):

1. **Problem and done criteria.** State both (1) a plain-language description of the end result (the typed-TOML `[meta] goal` key) and (2) a verifiable definition-of-done criterion for the task as a whole (`[meta] done_criterion`).
2. **Context.** Write for an executor who has **not** read the originating dialogue: define domain terms on first use, state why the task exists and the current-vs-target state. No references to conversation-only artifacts ("option B", "Q1", "as agreed") — inline the actual decision and its rationale so the plan stands alone. **Substantive plans must carry a plain-text `External research:` line here** (the typed-TOML equivalent is the `[meta] external_research` key) recording the § Research decision — what internet/intranet research was done and found, or one line on why it is not warranted; `agentctl.plan.load_plan` rejects a substantive plan that omits it. **Substantive plans must also carry the order, structurally** (`[meta.order]`): `customer_id`/`customer` (who asked, machine id + prose), `functional_place` (the place this plan's product fills and how its current filling is inadequate — the need), `requirements` (as `{id, text}` pairs), and `[meta.order.coverage]` mapping each requirement id to the control(s) that decide it, e.g. `R1 = ["stage 2 verify_command"]`. `check-order-coverage.py` resolves every coverage entry against a real stage `verify_command` / `final_check` index / landed-kind assertion — an unresolvable or absent entry is a defect a plan review must catch. Field-by-field rationale: [policy.md](policy.md) § Order.
3. **Stages.** Each stage is a full *elementary plan* whose constituents are the 8 activity elements — see [plan-activity-ontology](../../../memory-global/leaves/plan-activity-ontology.md). **Substantive** plans must declare all 8 per stage (the canonical model is `agentctl/plan.py`+`state.py`'s typed grouped structs — `Subject`/`Means`/`Actor`/`Criterion`/`Principle`/`Supply`/`Outcome`); lighter classes may omit them. If any element below is not a given, split it out as a **service stage** with a `depends_on` edge — any unmet element (material, means, even the actor/capability) becomes the order of a sub-plan, which is how a composite plan grows.
   ...
   - **Means & method:** reused tools / abstractions to extend (immutable means, element 4) and the REQUIREMENT on the way of acting — what the transformation must be an instance of: the algorithm / pattern to follow and where the change lands (file · symbol) (method, element 4'). This is the norm the executor is held to, so write what would make the result wrong if violated, not how to get there step by step.
   - **Procedure:** the ordered sequence of concrete sub-actions proposed for meeting that requirement, each naming the file / symbol it touches (element 4', the executor's half). Write the best sequence you can — and write it as a proposal: **the executor may replace the procedure** on his own reading of the code, via `agentctl replan --renormalize`, which the engine refuses the moment the edit reaches the method, the criterion, the result image or the goal. A sequence in `method` is therefore a norm nobody meant to set; a requirement in `procedure` is one anybody may drop.
   ...
   - **Knowledge & preconditions:** `knowledge` — what must already be known for the method over the means to reach the result (a place of its own, not a restatement of the principle or the means — or supply it from an earlier stage with `[[stage.supplies]] element = "knowledge"`). `preconditions` — the starting requirements the stage's `conditions` carry, kept apart from `conditions` for the same reason. `material_refs` / `knowledge_refs` — the structural projection of `material`/`knowledge`: the symbols this stage TRANSFORMS vs. the symbols it only READS. A symbol named in **both** is a smell — justify it in the stage's own prose rather than leaving it unexplained. Field-by-field rationale: [policy.md](policy.md) § Submission-seam fields.
   - **Expected result image:** *(definition of done — target state + control criterion, elements 2/3)* concrete observable + expected value/state (e.g. "`pytest tests/foo.py` exits 0", "table `users` has column `tier`"). For `measurable` — a runnable check; for `acceptance-review` — what the user inspects. `validate_planner_plan` (`scripts/lib/planner_plan_check.py`) requires ≥1 `Expected result image:` line. A measurable stage's `verify_command` (TOML plans) runs in the plan's `[meta] repo_root` when set (`cd repo_root && cmd`); unset, it inherits the engine-invoker's cwd, so its paths must then be **absolute** — set `repo_root` whenever the checks use repo-relative paths. **Two-directional control:** a control is trusted only when it can go **RED on a real mutation** *and* its **GREEN direction is reachable** — every literal path the `verify_command` / `final_check` names must either already exist or be produced by some stage. Declare a path a stage produces in that stage's `output_artifacts = [...]`; submit-plan **BLOCKS** a substantive plan whose control names a bare literal path that neither exists under `repo_root` nor appears in any stage's `output_artifacts` (a control that can never pass honestly is a broken control — reachability has no legitimate counter-instance, hence a blocker not a warning). Reachability is not validity: a green-reachable check still only means it *can* run, never that green *proves* the done criterion. **For a worktree-delivered change** (Core edits land via PR; the canonical checkout stays on main), set `[meta] delivery_worktree` and **declare** the venue — `verify_venue` on a stage, `venue` on a `[[final_check]]` — instead of hardcoding an absolute `cd` into that worktree: pre-landing the change is absent from `repo_root`, so a `repo_root`-anchored check observes the wrong tree and a genuinely-green delivery false-fails; the canonical-checkout run is the *post-landing* confirmation. **When the plan also lands the change**, those same stages additionally declare `verify_venue_at_final = "repo_root"` (plus `verify_kind = "landed"` to assert the delivery reached trunk) per `scripts/agentctl/README.md` § Check venue — a hardcoded `cd` buys none of that: `plan.check_venue_warnings` returns early while no venue is declared, so its survivability warning (landing removes the delivery worktree, so this check will refuse at verify-final) never fires and the plan fails its own final verification after a correct delivery. `submit-plan` warns (non-blocking) when a check mismatches the venue. **A verify literal quoting file content is read back from its target file:** a `verify_command` that greps for file content is produced by running that grep after the edit, never written from memory of a sibling paragraph — a literal that in fact landed in a neighbouring file turns a correct delivery RED.
...
Optional `##` sections (add when the task warrants them; not enforced by `agentctl.plan.load_plan`):
