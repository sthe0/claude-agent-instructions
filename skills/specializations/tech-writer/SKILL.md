...
7. **Ground a conclusion in a link to its evidence.** Every conclusion or remark that rests on a measured result — a benchmark number, a run outcome, a comparison, a cost figure — carries a link to the artifact that grounds it: the run/graph URL and its terminal status, the results table, the PR, the measurement. State the ground *where the claim is made*, not in a distant appendix; for a claim recovered from a failed-but-salvaged run, name the recompute method too, so the reader learns the run failed rather than reading it as clean success. An ungrounded conclusion reads as bare assertion and the reader cannot check it — this is the exposition side of the L1 claim-ledger (see the `formalization-ladder-l1-l3` leaf), and it pairs with the code-reference link-formatting rule (§ How you write, rule 9). *(Difficulty removed: a report whose conclusions cite no openable evidence is indistinguishable from confident guesswork — a reader cannot separate a measured claim from an invented one, and a failed run behind a salvaged number reads as "all green".)*
