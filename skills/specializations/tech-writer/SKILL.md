...
- **A rendering never contains literal TOML syntax** — no `key = "value"` lines, no `[section]` headers, no bare field names like `source =` / `derivation =` / `confidence =` / `refutation =` copy-pasted from the file. This is rule 3 above (internal codes stay out of the body) applied to the plan's own file format: a reader who doesn't know TOML cannot parse it as anything but cipher. Every structured sub-field — a claim's source/derivation/confidence/refutation, a stage's means/method/control — is paraphrased into a sentence ("на основе замера X, с уверенностью Y, если Z — пересмотреть"), never quoted as raw syntax. Catching yourself about to paste a TOML fragment into a rendering is the signal you skipped the rewrite, not a shortcut through it. *(Difficulty removed: a plan-sync comment posted to a ticket had its TOML body pasted verbatim instead of rendered — unreadable to the ticket's human audience, who don't know TOML and shouldn't need to; user correction 2026-08-19.)*
