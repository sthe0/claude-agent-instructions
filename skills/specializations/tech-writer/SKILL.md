...
4. **Concision is not amputation — and it applies evenly.** Cutting water must not cut the point. When you compress a list or a section, every item that stays must still carry its own essence — what it is and why it matters — not a teaser fragment that makes sense only to someone who already knows the content; a one-line stub that forces the reader into an appendix just to learn what the item *is* is over-compression, not brevity. And the standard covers the whole deliverable, not only the lead: detail pushed under a fold or into an appendix is still read by someone, so it gets the same line-level pass — a fold is a place for *detail*, not a dump for telegraphic notes. *(Difficulty removed: a report whose lead was polished shipped with its later sections compressed to intelligible-only-if-you-already-know stubs and its appendix left as raw telegraphic notes — the reader could not tell what each item was for.)*

5. **Level consistency in an enumeration.** When items are presented as a list of peers — plan steps, report sections, options, tracks — they must be the same *kind* of thing at the same level of subordination. An item that actually *serves, presupposes, qualifies, or bounds* another — a precondition, a guard on a headline result, a caveat, a scope-exclusion — is subordinate to it, not its sibling: nest it under its parent or demote it to a note, never inflate it into a peer. A subordinate dressed as a peer both misstates the structure (the reader cannot tell the load-bearing items from the qualifications on them) and reads as padding (the list looks longer and flatter than the work is). *(Difficulty removed: a report's "what we'll do" list carried two guards on the main number and an out-of-scope note as if they were peer workstreams; the reviewer flagged them one after another as contrived padding.)*

6. **Cut speculative water, but never cut a real risk.** Remove hedges that only multiply entities — `может пригодиться`, `держим наготове`, `на будущее`, `на случай если` — when they name no concrete consequence. But an explicit **risk**, a necessary **assumption**, or a known **problem** is load-bearing content: surface it, state it plainly, and set it apart (its own line or heading) rather than folding it away as if it were filler. The asymmetry is the rule — speculative padding goes, real caveats stay and get highlighted. *(Difficulty removed: a blanket "cut the water" pass either leaves the speculative hedges in or, worse, strips out the genuine risks and assumptions the reader most needs.)*

7. **Ground every checkable claim so the reader can verify it themselves.** Every conclusion or remark that rests on a specific fact the reader could look up — a benchmark number, a run or process outcome (including a *qualitative* one: it hung, it crashed, it timed out, it is still retrying — not only a numeric result), a comparison, a cost figure — carries one of three things: a **link** to the artifact that grounds it (the run/graph URL and its terminal status, the results table, the PR, the measurement, or the run/process ID when no URL exists); a **step-by-step recipe or script** the reader can run to reproduce it themselves; or a **link to where that recipe/script already lives**. State the ground *where the claim is made*, not in a distant appendix; for a claim recovered from a failed-but-salvaged run, name the recompute method too, so the reader learns the run failed rather than reading it as clean success. **A named-but-unlinked incident is not grounded** — "a test run hung" or "the full run failed" with no ID, link, or reproduction path is exactly as unverifiable as a bare benchmark number with no source; if you cannot supply one of the three, say so explicitly rather than asserting the incident as settled fact. An ungrounded conclusion reads as bare assertion and the reader cannot check it — this is the exposition side of the L1 claim-ledger (see the `formalization-ladder-l1-l3` leaf), and it pairs with the code-reference link-formatting rule (§ How you write, rule 9). *(Difficulty removed: a report whose conclusions cite no openable evidence is indistinguishable from confident guesswork — a reader cannot separate a measured claim from an invented one, a failed run behind a salvaged number reads as "all green", and a qualitative claim like "a run hung" shipped with zero ID because the rule's own wording read as scoped to numeric benchmark results — user correction 2026-09-02.)*

8. **Explain a designed thing by its functional grounds, not by an inventory of its parts.** When the text answers *how does X work* or *how does X handle Y*, lead with the constitutive concepts and, for each, the difficulty it removes; the mechanisms then read as consequences of those concepts and belong late, or in an appendix. This is stricter than rule 1 and than the outcome-format leaf's main-first: an inventory of components **ordered by importance** satisfies main-first while still failing the reader, who cannot tell which parts are load-bearing, cannot generalize the design, and cannot judge whether it answers the problem at all. It is the authoring-side application of the repo's own root principle (every rule has a functional ground) — a section describing a mechanism whose difficulty you cannot name is the same prune signal here as it is there. *(Difficulty removed: asked how the system handles a class of failures, the author answered with a catalogue of hooks, plugins and gates; every entry was correct and the reader still could not see the design, because the concepts generating those mechanisms were never stated — user correction 2026-08-10.)*
...
8. **Unpack a self-coined compound.** A multi-word nominal term the author *invented* — one with no standard meaning the reader can look up — reads as cipher; replace it with a plain description of what it *does*, don't just gloss it in place. E.g. «режим повторной выборки на один вход» → «прогонять каждую задачу по нескольку раз ради надёжности pass^k». *(Difficulty removed: an author-coined compound with no established meaning stalls the first-pass read exactly like un-introduced jargon — the reader has nowhere to decode it.)*
9. **Format a code reference by what it is.** In a reader-facing deliverable, a repository entity that has a stable link target renders as a **plain link, without surrounding backticks** — the link's colour and underline are what tell the reader it is clickable — and **every** occurrence is linked, not only the first (pin the link to a revision; see the `code-links-carry-revision` leaf). Reserve monospace **backticks** for things that are *not* navigable repo entities: metrics and formulas (`pass@k`, `nDCG`), generic or illustrative filenames, CLI fragments, external/non-repo paths. The reason is renderer-general: on any renderer where inline code inside a link inherits the surrounding colour, a link *wrapped* in backticks renders pixel-identical to plain monospace, so the reader cannot tell a clickable repo entity from mere jargon — pulling repo code out of backticks lets the renderer itself separate three tiers (coloured link = navigable repo entity / monospace = literal or jargon / plain text = prose). *(Difficulty removed: a repo link buried in backticks is invisible as a link, and monospace alone cannot signal whether a term is a navigable repo entity or just a literal.)*
10. **Verify every cited artifact is actually reachable by the reader.** Before returning `COMPLETED:` on a reader-facing deliverable (ticket comment, PR description, documentation page), scan the text for every cited file or path. A repo entity already covered by rule 9 (committed, revision-pinned) is fine. Anything else — a working-copy path, a `/tmp` scratch path, an untracked file — is not a reference for an outside reader unless it is *already* committed with a pinned revision or *already* attached/uploaded to the target and cited by that attachment's id/URL. Also scan for the **inverse** gap: a sentence naming a specific run, process, or incident ("a test run hung", "the pipeline failed") with **no** ID, link, or reproduction path attached anywhere nearby — this is a rule-7 violation, but the reachability scan is the right place to catch it, since both checks read the same text in one pass. If a cited artifact fails this test, do not ship the citation: name it in your `COMPLETED:`/`INCOMPLETE:` note so the manager attaches or commits it first, or the wording changes to stop promising access it doesn't have. This is the polish-pass enforcement point for the `outcome-format` leaf's rule that a local filesystem path is not a reference for an external reader — the tech-writer pass is the last checkpoint before publish, so it is where this check has to actually fire, not just where it is documented. *(Difficulty removed: the rule already existed in memory, but nothing in the tech-writer's own pre-publish checklist re-checked it at the moment that mattered, so a ticket comment shipped citing an unattached scratch file as if it were a durable reference — user correction 2026-08-23.)*
