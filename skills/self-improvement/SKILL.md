...
4. **Classify.** Reasoning error / missing tool / wrong delegation / stale or misplaced memory / noise in instructions / wrong place for the content — and behavioral rule vs domain fact (§ Behavioral rule vs domain fact): a rule → instructions, a fact → memory leaf. **A `replan` is one of two distinct things — do not conflate them.** Either (a) a **planning-ability signal** — the plan was wrong or incomplete and a difficulty surfaced it: *this* is a valid self-improvement target, re-norm the planning habit that missed it; or (b) a **user-sanctioned scope expansion** — the user explicitly widened the task mid-flight (a new approved directive): this is a normal approved change, **not** a planning failure — do not re-norm against it, do not record it as a planning-quality lesson, do not treat the mere fact of a replan as agent-fault. Attribute the replan to its cause before deciding whether self-improvement even applies. *(Difficulty removed: a user-driven scope change read as a planning miss produces a spurious self-correction that punishes the agent for doing exactly what the user asked.)*
...
**Structural form before prose — for a deterministically-decidable rule.** When the behavioral rule is decidable from observable inputs (an ordering, a classification, a gate, a validation, a dispatch — its *rule* part, as opposed to a *perception* part only the model can supply), do not settle for adding prose. Per the CLAUDE.md root principle *"Separate rule from perception; determinize the rule at its proper structural level"*, propose mechanizing the rule part at its proper structural level (engine state / state machine / typed contract / gate / hook) and keep prose only for the perception the model genuinely owns — and **propose that structural form yourself**, do not wait for the user to ask. A local hook or script is a stopgap until a structural home exists; a one-off crutch when a reusable primitive fits is a prune candidate, not the target. **Tie-breaker:** when an existing script/hook already implements the rule for one path, extending that mechanism to the missing path is the **default** proposal — prose-only needs a named reason it wins (a prose-only beat-1 for a mechanizable rule is the failure mode this sentence removes; user correction 2026-07-03 "Лучше кодом"). **A regex driving a HARD BLOCK on a natural-language / semantic classification is the specific anti-pattern this covers:** demote the regex to a high-recall prefilter and let a fail-open model judge make the semantic call — `agentctl/advisor.py::judge_binary_ask` is the in-repo template (prefilter → model → fail-open); see [regex-not-for-semantic-classification](../../memory-global/leaves/regex-not-for-semantic-classification.md).
