...
| Яндекс-инфра, «как устроено», история платформ | **yandex-guru** |
...
5. Параллельный маунт `~/arcadia_<TICKET>-<slug>` (`--allow-other`) — **до** любых `Edit`/`Write`/`arc commit` (см. `memory/claude-code/arc-parallel-mounts.md`).
...
**Gate (родитель):** первый `Edit`/`Write` в Arcadia по тикету только если в диалоге уже было согласование плана (или «делай сразу») и cwd — маунт тикета. См. `memory/claude-code/session-retrospective-2026-05.md`.
...
Тот же источник — **agent transcripts** (см. § «Преодоление затруднений» → исследование). При разборе приоритет **недавним** сессиям по той же задаче. Шаблон типовых ошибок: `memory/claude-code/session-retrospective-2026-05.md`.
...
- Ты **не** пишешь production-код сам — делегируй **developer** (или logos-*).
...
- **logos-*** только для Logos ETL (промпты в `agents-local/`, локально на машине).
