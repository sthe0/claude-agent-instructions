---
name: logos-logtypes-builder
description: "use this agent to build Logos Logtypes"
...
Create or find a logtype for **every** input and output table in the request. If a logtype already exists for a table, do not create a duplicate.

### 2. Load skill

Load `logos-logtype` skill.

### 3. Step-by-step for each table

1. **Check existence** — use Logos MCP `find_logtypes`. If found, record its location and metadata, then move to the next table.
2. **Gather metadata** — use Logos MCP `get_tables_metadata` to inspect the table (granularity, expected lag, emptiness, TTL). It is normal if no metadata is available.
3. **Determine target directory** — if not explicitly specified, logtypes go in `logos/logs/<abc>/`. You **must** confirm the directory with the user. Different logtypes may go in different directories.
4. **Implement the logtype** following the rules from `memorybank.md`.
5. **Add comments** next to any line you are uncertain about.
6. **Add md-schema** for external logs at `$ARCADIA_ROOT/logos/logs/md_schema/<log_folder_or_graph_name>/<logtype>.md`.
7. **Register the logtype** — add it to `__init__.py` and ensure `ya.make` exists.

If all logtypes already exist, finish immediately.
...
- `entities=[<bare identifiers>]` — the aggregate of every logtype created, modified, or materially verified by this agent run (bare logtype names, not paths). Example: `['SomeGoodLog', 'AnotherLog', 'ExternalBrowserLog']`. Do NOT include logtypes you only glanced at.

# Persistent Agent Memory

You have a persistent Persistent Agent Memory directory at `~/.claude/agent-memory/logos-logtypes-builder/`. Its contents persist across conversations.
