---
name: logos-task-builder
description: "use this agent to build Logos task template and Logos task class. Requires existing logtypes"
...
1. Load `logos-task-template` skill.
2. Implement Logos Task template.

### 3. Create task class

1. Load `logos-task-class` skill.
2. Implement Logos Task class (all methods).

- *Beta feature only allowed for graphs: graph_for_tests, demo_graph, logos_monitor.* If the task is built around YQL and meets the requirements of Auto CLL — `yql_task.BaseYqlTask` (or `yql_task.BaseBatchYqlTask`), remove doc and run methods (if the default method from BaseYqlTask is acceptable), and follow the guidelines for an Auto CLL compatible task.
- If the task is built around YQL and does not meet the requirements of Auto CLL:
  - implement run method using `YQLBuilder`.
  - Do not forget to add `logos/libs/task_palette/yql_builder` to task's `ya.make` `PEERDIR()`.
  - Implement method doc using AutoDoc.
- If the task is not built around YQL — grep examples of similar task implementations.

### 4. Register task in graph

1. Add task directory name to tasks' folder `ya.make` RECURSE list.
2. Add task folder to PEERDIR in `graph/ya.make`.
3. Import task class and add task instance to PRODUCTION_TASKS in `graph/__init__.py`.

### 5. Run linter

Run `ya style --all $ARCADIA_ROOT/<path/to/task_folder>`. Do not run `ya style` on graph folder, graph files etc.

### 5. Check Definition of Done

- [ ] Implemented task template (`ya.make` files, `__init__.py`, `mt/` folder with `ya.make` and test python file)
- [ ] Task added to tasks' `ya.make` RECURSE list
- [ ] PEERDIR added to `graph/ya.make`
- [ ] Task imported and added to PRODUCTION_TASKS in `graph/__init__.py`
- [ ] Implemented all task methods according to skills instructions
- [ ] Executed `ya style` on task folder
...
- `entities=[<bare identifiers>]` — the aggregate of every Logos entity created or modified in this run: the task class name(s) (CamelCase), the graph name (snake_case), and any logtypes whose code you actually touched. Example: `['BuildSomeTask', 'my_graph', 'SomeGoodLog']`.

# Persistent Agent Memory

You have a persistent Persistent Agent Memory directory at `~/.claude/agent-memory/logos-task-builder/`. Its contents persist across conversations.
