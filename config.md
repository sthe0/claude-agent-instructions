...
| `core-difficulty-mass-threshold` | `<calibrated in stage 13>` | Cluster mass (Σ severity, optionally recency-decayed) at/above which a Core-difficulty cluster is flagged for a batched Core change (ADR-0001 § Difficulty-accumulation). Consumed by the digest `core-difficulty-digest.py` (added under `scripts/` in stage 9). **Value deferred** to ADR-0001 open question #2 — set by the stage-13 calibration with a documented basis; never guessed (see the `is_author` row's principle). |
| `is_author` | `false` | Per-machine boolean: does this machine hold Core commit authority? Default `false` — a machine is a non-author until granted. Consumed by `self-improvement` (a non-author routes a Core-target difficulty to the `DifficultyChannel` instead of editing Core) and by the author-side `core-difficulty-digest.py`. Authority may also be probed via `git push --dry-run` capability. |
