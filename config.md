...
| `difficulty-channel` | `github` | Channel for filing Core difficulties from non-author machines, **auto-detected** per machine by `scripts/difficulty_channel/detect.py` (strong internal Yandex signals → `startrek`; otherwise → `github`) and written to `~/.claude/agent-identity.local` (`difficulty_channel=<name>`); override by editing that line. Values: `github` (GitHub Issues sthe0/claude-agent-instructions — the org-neutral default) or `startrek` (internal Yandex Tracker queue OOSEVENREPORT — opt-in, requires `~/.tracker-token`). Core commit authority is determined solely by `git push --dry-run` capability — no per-machine config flag (a shared config file cannot distinguish machines). |
